...
{
    public partial class FormPartyLedgerReport : System.Web.UI.Page
    {
        ...
        private void LoadGridData()
        {
            ...
            string rptTypeCnf = SQLQuery.ReturnString(@"");

            string lName = Page.User.Identity.Name.ToString();
            string prjID = "1";
            string supplierId = Convert.ToString(Request.QueryString["PartyId"]);
            string reporType = Convert.ToString(Request.QueryString["PartyType"]);
            ...
            rpt.Load(Server.MapPath("CrptPartyLedger.rpt"));
        ...
        private DataTable LoadGridData(string Customer, string dtFrom, string dtTo, string partyType)
        {
            string customer = " ";
            string party = " ";
            decimal opBal = 0;
            ...
            //get opening balance        
            string query = " where TrType='"+ partyType + "' AND TrGroup ='ImportLC'" + customer + opDate;
            decimal currBal = opBal + Convert.ToDecimal(SQLQuery.ReturnString("SELECT isnull(sum(Cr),0)-isnull(sum(Dr),0) FROM [Transactions] " + query));

            query = " where TrType='" + partyType + "' AND TrGroup ='ImportLC'" + customer + closeDate;
            decimal closeBal = opBal + Convert.ToDecimal(SQLQuery.ReturnString("SELECT isnull(sum(Cr),0)-isnull(sum(Dr),0) FROM [Transactions] " + query));
            ...
            query = " where  TrType = '" + partyType + "' AND TrGroup ='ImportLC'" + customer + invDate + closeDate;
