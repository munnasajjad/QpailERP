...
{
    public partial class WastageStockRegister : System.Web.UI.Page
    {
        ...
        private void LoadGridData()
        {
            ...
            dsx.Load(dr2, LoadOption.OverwriteChanges, dsx.WastageRegister);

            rpt.Load(Server.MapPath("CrptWastageRegister.rpt"));
        ...
        private DataTable BindItemGrid(string ddGodown, string ddLocation, string ddPurpose, string ddcategory, string ddGrade, string ddSubGroup, string ddType, string dateForm, string AsOn)
        {
            ...
            try
            {
                ...
                string query = " FROM Stock where EntryId<>0 " + godown + location + subGroup + dateFrom + dateTo;
                string url = "";// SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") + "PO-Print.aspx?pono=";

                ds = SQLQuery.ReturnDataSet(@"SELECT DISTINCT (SELECT GradeName FROM ItemGrade WHERE GradeID =(SELECT GradeID FROM Categories WHERE CategoryID =(SELECT CategoryID FROM Products WHERE (ProductID = Stock.ProductID)))) AS Grade, " +
                                            ...
                                            "  ISNULL(SUM(InWeight),0) AS Received, ISNULL(SUM(OutWeight),0) AS Issued, ISNULL(SUM(InWeight),0) - ISNULL(SUM(OutWeight),0) AS Balance, Stock.EntryDate AS Date " + query + " " + "GROUP BY WarehouseID, Purpose, ProductID, Stock.EntryDate");
