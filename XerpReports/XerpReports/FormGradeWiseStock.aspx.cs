...
{
    public partial class FormGradeWiseStock : System.Web.UI.Page
    {
        ...
        private void LoadGridData()
        {
            ...
            rpt.Load(Server.MapPath("CrptGradeWiseStock.rpt"));
        ...
        private DataTable LoadGridData(string category, string dtFrom, string dtTo, string itemId)
        {
            ...
            dt1.Columns.Add(new DataColumn("IssuedQty", typeof(decimal)));
            dt1.Columns.Add(new DataColumn("ReceivedQty", typeof(decimal)));
            ...
            decimal currBal = opQty + Convert.ToDecimal(SQLQuery.ReturnString("SELECT isnull(sum(IssuedQty),0)-isnull(sum(ReceivedQty),0) FROM vwGradeWiseStock" + query));

            query = " WHERE  ProductID = " + item  + cate2  + closeDate ;
            decimal closeBal = opQty + Convert.ToDecimal(SQLQuery.ReturnString("SELECT isnull(sum(IssuedQty),0)-isnull(sum(ReceivedQty),0) FROM vwGradeWiseStock" + query));
            ...
            if (recordcount > 0)
            {
                do
                {
                    ...
                    issued = Convert.ToDecimal(ds.Tables[0].Rows[ic]["IssuedQty"].ToString());
                    received = Convert.ToDecimal(ds.Tables[0].Rows[ic]["ReceivedQty"].ToString());
                    ...
                    dr1["IssuedQty"] = issued;
                    dr1["ReceivedQty"] = received;
                    dr1["Balance"] = currBal;
