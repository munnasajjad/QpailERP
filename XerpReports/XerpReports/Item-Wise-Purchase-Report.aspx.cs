...
{
    public partial class Item_Wise_Purchase_Report : System.Web.UI.Page
    {
        ...
        private void LoadGridData()
        {
            ...
            rpt.Load(Server.MapPath("CrptItem-Wise-Purchase-Report.rpt"));
...

        {
            try
            {
                ...
                query += "";
//                string str = @"SELECT DISTINCT ItemName, MAX(Price) AS MaxPrice, MIN(Price) AS MinPrice, MAX(EntryDate) AS LastDate,
//                             (SELECT        BillDate
//                               FROM            Purchase
//                               WHERE        (InvNo =
//                                                             (SELECT        InvNo
...
//                                                                                               FROM            PurchaseDetails AS PurchaseDetails_2
//                                                                                               WHERE        (ItemCode = '3239')))))) AS BillDate
//FROM            PurchaseDetails AS PurchaseDetails_1
//                        where " + query + "  Purchase.BillDate >= '" + dateFrom + "' AND Purchase.BillDate <= '" + dateTo +
//                             "' ORDER BY ItemName, ItemCode ";

                string str = @"SELECT DISTINCT ItemName, MAX(Price) AS HighestRate, MIN(Price) AS LowestRate, MAX(EntryDate) AS LastPurchaseDate,
                             (Select Price FROM PurchaseDetails WHERE (Id = (SELECT MAX(Id) AS Expr1 FROM PurchaseDetails WHERE (ItemCode = " + ddItemName + @")))) AS LastRate,
                             (SELECT BillDate FROM Purchase WHERE (InvNo = (SELECT InvNo FROM PurchaseDetails
                                                            WHERE (Id = (SELECT MAX(Id) AS Expr1 FROM PurchaseDetails AS PurchaseDetails_2 WHERE (ItemCode = " + ddItemName + @")))))) AS BillDate
                             FROM PurchaseDetails AS PurchaseDetails_1 WHERE (ItemCode = " + ddItemName +@") AND (InvNo IN
                             (SELECT InvNo FROM Purchase AS Purchase_1 WHERE (BillDate >= '" + dateFrom + "') AND (BillDate <= '" + dateTo +"'))) GROUP BY ItemName";
