...
{
    public partial class FormTradePayable : System.Web.UI.Page
    {
        ...
        private void LoadGridData()
        {
            ...
            DataTable dtx = RunQuery.SQLQuery.ReturnDataTable(@"SELECT Distinct (Select Company from Party where partyid=Transactions.HeadID) as Customer,
                        (Select OpBalance from Party where partyid=Transactions.HeadID)+ isnull(sum(Dr),0)-isnull(sum(Cr),0) as Balance FROM [Transactions] 
                         where  TrType = 'Supplier'   AND  (TrDate <= '" + dateFrom + "')  group by headid having ((Select OpBalance from Party where partyid=Transactions.HeadID)+ isnull(sum(Dr),0)-isnull(sum(Cr),0))<>0 order by balance desc ");
            ...
            rpt.Load(Server.MapPath("CrptTradePayable.rpt"));
