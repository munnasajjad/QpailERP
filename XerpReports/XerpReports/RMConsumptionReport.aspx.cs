...
{
    public partial class RMConsumptionReport : System.Web.UI.Page
    {
        ...
        private void LoadGridData()
        {
            ...
            rpt.Load(Server.MapPath("CrptConsPrdnPpcpOnly.rpt"));
        ...
        private decimal GetEquity(string rtype, string dateFrom, string dateTo)
        {
            ...
            InsertRptProduction(sLNo, "Opening Balance", "Total Opening Balance", Warehouse, Purpose, Grade, Category, "Production Floor (PPCP mixture)", Weight.ToString());
            Weight = 0;
            ...
            InsertRptProduction(sLNo, "Opening Balance", "Total Opening Balance", Warehouse, Purpose, Grade, Category, "Usuable Wastage (PPCP)", Weight.ToString());
            Weight = 0;
            ...
            dtx = SQLQuery.ReturnDataTable(@"SELECT DISTINCT (SELECT StoreName FROM Warehouses WHERE (WID = Stock.WarehouseID)) AS Warehouse, 
                (SELECT GradeName FROM ItemGrade WHERE GradeID =(SELECT GradeID FROM Categories WHERE CategoryID =(SELECT CategoryID FROM Products WHERE (ProductID = Stock.ProductID)))) AS Grade,
				(SELECT CategoryName FROM Categories WHERE CategoryID =(SELECT CategoryID FROM Products WHERE (ProductID = Stock.ProductID))) AS Category,  (SELECT ItemName FROM Products WHERE (ProductID = Stock.ProductID)) AS ProductName,
				ISNULL(SUM(InWeight),0) AS Weight  FROM Stock where EntryId<>0 AND (WarehouseID = '8') AND ProductID IN (Select ProductID from Products where CategoryID ='1411')  AND EntryDate>='" + dateFrom + "'  AND  EntryDate<='" + dateTo + "' GROUP BY WarehouseID, ProductID HAVING (ISNULL(SUM(InWeight), 0) - ISNULL(SUM(OutWeight), 0) <> 0)");
            foreach (DataRow drx in dtx.Rows)
            {
                Warehouse = drx["Warehouse"].ToString();
                Purpose = "";
                ...
                InsertRptProduction(sLNo, "Current Month Addition/Issued", "Total Issued", Warehouse, Purpose, Grade, Category, ProductName, Weight.ToString());
            ...
            foreach (DataRow drx in dtx.Rows)
            {
                ...
                InsertRptProduction(sLNo, "Total Raw Material", "Total Raw Material", Warehouse, Purpose, Grade, Category, ProductName, Weight.ToString());
            ...
            foreach (DataRow drx in dtx.Rows)
            {
                ...
                {
                    InsertRptProduction(sLNo, "Consumption/ Production PPCP", "Total Consumption", Warehouse, Purpose, Grade, Category, ProductName, Weight.ToString());
                }
            }
            ...
            string ttlCons = SQLQuery.ReturnString("SELECT ISNULL(SUM(Weight), 0) AS Weight FROM rptProduction WHERE GroupName = 'Consumption/ Production PPCP'");
            dtx = SQLQuery.ReturnDataTable(@"SELECT ISNULL(SUM(Weight),0) AS Weight FROM rptProduction WHERE GroupName = 'Opening Balance' OR GroupName = 'Current Month Addition/Issued'");
            foreach (DataRow drx in dtx.Rows)
            {
                ...
                Grade = "";
                Category = "";
                ProductName = "PPCP mixture at Floor";
            ...
            foreach (DataRow drx in dtx.Rows)
            {
                ...
                Grade = "";
                Category = "";
                ProductName = "Actual PPCP mixture at Floor found during Inventory";
                Weight = 0;
                InsertRptProduction(sLNo, "Actual Material Remaining at Production Floor", "Difference between Theoretical & Physical stock", Warehouse, Purpose, Grade, Category, ProductName, Weight.ToString());
