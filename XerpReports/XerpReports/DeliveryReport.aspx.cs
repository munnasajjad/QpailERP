...
namespace Oxford.XerpReports
{
    public partial class DeliveryReport : System.Web.UI.Page
    {
        ...
        private DataTable BindItemGrid(string dateFrom, string dateTo, string cust)
        {
            ...
            try
            {
                ...
                if (dateFrom != "")
                {
                    dateFrom = " AND InvDate>='" + Convert.ToDateTime(dateFrom).ToString("yyyy-MM-dd") + "' ";
                }

                if (dateTo != "")
                {
                    dateTo = " AND InvDate<='" + Convert.ToDateTime(dateTo).ToString("yyyy-MM-dd") + "' ";
                }

                if (cust != "---ALL---")
                {
                    cust = " AND CustomerID='" + cust + "' ";
                }
                else
                {
                    cust = "";
                ...
                //}
                string query = " " + dateFrom + dateTo + cust;
                string url = "";// SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") + "PO-Print.aspx?pono=";

                query = @"SELECT DISTINCT SaleDetails.ItemChallanNo, Sales.InvDate AS PODate, SUM(SaleDetails.Quantity) AS Quantity, Sales.CustomerID
...
WHERE  SaleDetails.Id<>0 " + query + " GROUP BY Sales.InvDate, SaleDetails.ItemChallanNo, Sales.CustomerID  ORDER BY Sales.InvDate";
        ...
        private void LoadGridData()
        {
            ...
            DataTable dt1 = BindItemGrid(dateFrom, dateTo, cust);
            DataTableReader dr2 = dt1.CreateDataReader();

            XerpDataSet dsx = new XerpDataSet();
            dsx.Load(dr2, LoadOption.OverwriteChanges, dsx.DeliveryChallan);

            rpt.Load(Server.MapPath("CrptDeliveryReport.rpt"));
            ...
            if (cust!="---ALL---")
            {
                rptName = SQLQuery.ReturnString("SELECT [Company] FROM [Party] WHERE PartyID='" + cust + "'");
            }
