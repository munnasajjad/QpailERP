...
{
    public partial class FromVoucherEditHistory : System.Web.UI.Page
    {
        ...
        private void LoadGridData()
        {
            ...
            DataTable dtx = RunQuery.SQLQuery.ReturnDataTable(@"SELECT     VoucherNo, VoucherDate, VoucherDescription, EditBy, EditDate, VoucherAmount, VoucherRowDescription, AccountsHeadName, VoucherDR, VoucherCR
FROM            vw_VoucherEditHistory ORDER BY VoucherNo");

            DataTableReader dr2 = dtx.CreateDataReader();

            XerpDataSet dsx = new XerpDataSet();
            dsx.Load(dr2, LoadOption.OverwriteChanges, dsx.vw_VoucherEditHistory);

            rpt.Load(Server.MapPath("CrptVoucherEditHistory.rpt"));
