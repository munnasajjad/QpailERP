...
{
    public partial class Coll_Heads : System.Web.UI.Page
    {
            ...
            try
            {
                if (txtName.Text != "")
                {
                    if (btnSave.Text == "Save")
                    {
                        ...
                        cmd.CommandText = "Insert into CollectionHeads (GroupID, Name, Description, Amount, Entryby) Values (@GroupID, @Name, @Description, @Amount, @Entryby)";
                    ...
                    {
                        SqlCommand cmd = new SqlCommand("UPDATE CollectionHeads SET GroupID=@GroupID, Name=@Name, Description=@Description, Amount=@Amount where (sl ='" + DropDownList1.SelectedValue + "')", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        {
            SqlCommand cmd7 = new SqlCommand("SELECT GroupID, Name, Description, Amount FROM [CollectionHeads] WHERE sl='" + DropDownList1.SelectedValue + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
