        ...
        private void PrintStudentID(string query)
        {
            try
            {
                ...
                foreach (DataRow row in dt.Rows) //Each Items Will be looped
                {
                    ...
                    string fatherName = row["FatherNameE"].ToString();
                ...
                Page.ClientScript.RegisterStartupScript(this.GetType(), "OpenWindow", "window.open('./Reports/rptIdCard.aspx','_newtab');", true);
