...
{
    public partial class ClassSetup : System.Web.UI.Page
    {
            ...
            try
            {
                if (txtName.Text != "")
                {
                    if (btnSave.Text == "Save")
                    {
                        ...
                        cmd.CommandText = "Insert into Class (name, NameNumeric, TutionFee, ClassTeacher, IDFormat) Values (@name, @NameNumeric, @TutionFee, @ClassTeacher, '" + txtIDFormat .Text+ "')";
                        ...
                        if (txtName.Text != "")
                        {
                            ...
                            if (Success > 0)
                            {
                                lblMsg.Text = "New Class Successfully Saved!";
                    ...
                    {
                        SqlCommand cmd2 = new SqlCommand("UPDATE Class SET Name=@Name, NameNumeric=@NameNumeric, TutionFee=@TutionFee, ClassTeacher=@ClassTeacher, IDFormat='"+txtIDFormat.Text+"' where (sl ='" + DropDownList1.SelectedValue + "')", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        {
            SqlCommand cmd7 = new SqlCommand("SELECT sl, name, NameNumeric, TutionFee, ClassTeacher, IDFormat FROM [Class] WHERE sl='" + DropDownList1.SelectedValue + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
            ...
            if (dr.Read())
            {
                ...
                txtIDFormat.Text = dr[5].ToString();
            }
