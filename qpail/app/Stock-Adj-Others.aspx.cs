...
public partial class app_Stock_Adj_Others : System.Web.UI.Page
{
    ...
    private void InsertData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO StockinDetailsRaw (StockType, OrderID, SizeId, ProductID, ProductName, BrandID, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor, GodownID, LocationID, QtyBalance, DeliveredQty) " +
                                         "VALUES ('Others', @OrderID, @SizeId, @ProductID, @ProductName, @BrandID, @UnitCost, @Quantity, @UnitWeight, '" + ltrUnit.Text + "', @ItemTotal, @TotalWeight, @EntryBy, @CompanyFor, '" + ddGodown.SelectedValue + "', '"+ddLocation.SelectedValue+"','" + balance + "', '" + ltrCQty.Text + "'  )", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    private void BindItemGrid()
    {
        ...
        SqlCommand cmd = new SqlCommand(@"SELECT Id, 
            ...
            FROM StockinDetailsRaw WHERE GodownID='" + ddGodown.SelectedValue + "' AND LocationID = '"+ddLocation.SelectedValue+"' AND EntryBy=@EntryBy AND OrderID='' AND StockType='Others'  ORDER BY Id", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    private void ExecuteInsert()
    {
        ...
        SqlCommand cmd = new SqlCommand("UPDATE StockinDetailsRaw SET OrderID='" + orderNo + "' WHERE  EntryBy=@EntryBy AND OrderID=''  AND StockType='Others'  AND GodownID='" + ddGodown.SelectedValue + "'  AND LocationID = '" + ddLocation.SelectedValue + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    private void QtyinStock()
    {
        try
        {
            ...
            txtCurrentQty.Text =
                Stock.Inventory.NonUsableQty(ddProduct.SelectedValue, ddGodown.SelectedValue, ddLocation.SelectedValue);
    ...
    {
        SqlCommand cmd = new SqlCommand(@"SELECT ItemType, ProductID, SizeId, Customer, BrandID, Color, Quantity, UnitCost, GodownID, LocationID, ProductName, CompanyFor FROM [StockinDetailsRaw] WHERE Id='" + entryID + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Connection.Open();
