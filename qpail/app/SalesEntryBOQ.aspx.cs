...
public partial class app_SalesEntryBOQ : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            ddAddress.DataBind();
            ddSize.DataBind();
            ComponentGrid.EmptyDataText = "No Data";
            GeneratePartyDetail();
            BindItemGrid();
            BindConsumedItemGrid();
            BindComponentGrid();
    ...
    private void ExecuteInsert()
    {
        ...
        int salesDetailsMasterId = Convert.ToInt32(SQLQuery.ReturnString("SELECT MAX(Id) AS MaxId FROM SaleDetails"));

        //Cost of Goods Sold
        decimal totalConsumedAMount = InsertCostOfGoodsSoldData(salesDetailsMasterId, orderNo);
        SQLQuery.ExecNonQry("UPDATE BOQConsumption SET SalesDetailsId = '" + salesDetailsMasterId + "', InvoiceNo = '" + orderNo + "' WHERE InvoiceNo = '' AND EntryBy = '" + lName + "'");
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO Sales (InvNo, InvDate, SalesMode, CustomerID, CustomerName, PONo, PODate, Period, DeliveryDate, DeliveryTime, DeliveryLocation, TransportDetail, MaturityDays, OverDueDate, " +
                                         " Remarks, InvoiceTotal, ProfitAmount, VATPercent, VATAmount, PayableAmount, CartonQty, ChallanNo, CollectedAmount, DueAmount, ProjectID, EntryBy, IsActive, InWords, Warehouse, VatChalNo, VatChalDate, InvCompany, ManufacturingDate, ExpiryDate)" +
        ...
        cmd2.Parameters.AddWithValue("@ProfitAmount", Convert.ToDecimal(txtTotal.Text) - totalConsumedAMount);
    ...
    protected bool IsEmptyComponentSection()
    {
        string isExist = SQLQuery.ReturnString(
             "SELECT COUNT(*) CountData FROM BOQConsumptionTemp WHERE ComponentId <> '0'");
        if (isExist == "0")
        { return false; }
        return true;
    ...
    protected bool VerifyComponentData()
    {
        //SQLQuery.ExecNonQry("TRUNCATE TABLE BOQConsumptionTemp");
        List<string> componentList = new List<string>();

        foreach (GridViewRow row in ComponentGrid.Rows)
        {
            DropDownList componentDropDownList = row.FindControl("ddComponent") as DropDownList;
            if (componentDropDownList.SelectedValue != "0")
            {
                //SqlCommand cmd =
                //    new SqlCommand(
                //        "INSERT INTO BOQConsumptionTemp (ComponentId, ComponentName) VALUES (@ComponentId, @ComponentName)",
                //        new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));

                //cmd.Parameters.AddWithValue("@ComponentId", componentDropDownList.SelectedValue);
                //cmd.Parameters.AddWithValue("@ComponentName", componentDropDownList.SelectedItem.Text);

                //cmd.Connection.Open();
                //cmd.ExecuteNonQuery();
                //cmd.Connection.Close();

                componentList.Add(componentDropDownList.SelectedValue);
            }
        }

        //SqlCommand command = new SqlCommand(
        //    "SELECT COUNT(*) AS CountDuplicate FROM BOQConsumptionTemp WHERE ComponentId <> '0' GROUP BY ComponentId HAVING COUNT(*) > 1 ORDER BY COUNT(*) DESC",
        //    new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        //{ return false; }
        //return true;
        var count = componentList.GroupBy(item => item)
            .Where(item => item.Count() > 1)
            .Sum(item => item.Count());



        if (count > 0 || componentList.Count == 0)
        { return false; }
        return true;
    }
        ...
        try
        {
            if (btnAdd.Text == "Add")
            {
                ...
                //{
                    ...
                    {
                        Notify("ERROR: No component data added or duplicate data added!", "warn", lblMsg);
                    }
                //}
                //else
                //{
    ...
    private void InsertData()
    {
        ...
        decimal totalComponentAmount = 0;
        foreach (GridViewRow row in ComponentGrid.Rows)
        {
            TextBox txtComponentQuantity = row.FindControl("txtComponentQuantity") as TextBox;
            DropDownList componentDropDownList = row.FindControl("ddComponent") as DropDownList;

            if (componentDropDownList.SelectedValue != "0")
            {
                decimal componentRate = Convert.ToDecimal(SQLQuery.ReturnString(
                    "SELECT TOP(1) ISNULL(MAX(InValue), 0) AS InValue FROM tblFifo WHERE ItemCode = '" + componentDropDownList.SelectedValue + "'"));
                decimal consumedAmount = Convert.ToDecimal(txtComponentQuantity.Text) * componentRate;
                totalComponentAmount += consumedAmount;
                SqlCommand cmd = new SqlCommand("INSERT INTO BOQConsumption (SalesDetailsId, InvoiceNo, CompanyId, CompanyName, ProductId, ProductName, ComponentId, ComponentName, ConsumptionQuantity, Rate, ConsumedAmount, EntryBy, EntryDate)" +
                                                " VALUES (@SalesDetailsId, @InvoiceNo, @CompanyId, @CompanyName, @ProductId, @ProductName, @ComponentId, @ComponentName, @ConsumptionQuantity, @Rate, @ConsumedAmount, @EntryBy, @EntryDate)", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));

                cmd.Parameters.AddWithValue("@SalesDetailsId", "");
                cmd.Parameters.AddWithValue("@InvoiceNo", "");
                cmd.Parameters.AddWithValue("@CompanyId", ddCustomer.SelectedValue);
                cmd.Parameters.AddWithValue("@CompanyName", ddCustomer.SelectedItem.Text);
                cmd.Parameters.AddWithValue("@ProductId", ddProductName.SelectedValue);
                cmd.Parameters.AddWithValue("@ProductName", ddProductName.SelectedItem.Text);
                cmd.Parameters.AddWithValue("@ComponentId", componentDropDownList.SelectedValue);
                cmd.Parameters.AddWithValue("@ComponentName", componentDropDownList.SelectedItem.Text);
                cmd.Parameters.AddWithValue("@ConsumptionQuantity", Convert.ToDecimal(txtComponentQuantity.Text));
                cmd.Parameters.AddWithValue("@Rate", componentRate);
                cmd.Parameters.AddWithValue("@ConsumedAmount", consumedAmount);
                //cmd.Parameters.AddWithValue("@Remarks", consumedAmount);
                cmd.Parameters.AddWithValue("@EntryBy", lName);
        ...
        decimal packingMaterialsRate = Convert.ToDecimal(SQLQuery.ReturnString(
                    "SELECT TOP(1) ISNULL(MAX(InValue), 0) AS InValue FROM tblFifo WHERE ItemCode = '" + ddPackingItems.SelectedValue + "'"));
        decimal totalBagRequired = Math.Ceiling(Convert.ToDecimal(txtQty.Text) / Convert.ToDecimal(txtQuantityPerBag.Text));
        decimal packingMaterialsConsumedAmount = totalBagRequired * packingMaterialsRate;
        totalComponentAmount += packingMaterialsConsumedAmount;
        SqlCommand command = new SqlCommand("INSERT INTO BOQConsumption (SalesDetailsId, InvoiceNo, CompanyId, CompanyName, ProductId, ProductName, ComponentId, ComponentName, ConsumptionQuantity, Rate, ConsumedAmount, EntryBy, EntryDate)" +
                                        " VALUES (@SalesDetailsId, @InvoiceNo, @CompanyId, @CompanyName, @ProductId, @ProductName, @ComponentId, @ComponentName, @ConsumptionQuantity, @Rate, @ConsumedAmount, @EntryBy, @EntryDate)", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));

        command.Parameters.AddWithValue("@SalesDetailsId", "");
        command.Parameters.AddWithValue("@InvoiceNo", "");
        command.Parameters.AddWithValue("@CompanyId", ddCustomer.SelectedValue);
        command.Parameters.AddWithValue("@CompanyName", ddCustomer.SelectedItem.Text);
        command.Parameters.AddWithValue("@ProductId", ddProductName.SelectedValue);
        command.Parameters.AddWithValue("@ProductName", ddProductName.SelectedItem.Text);
        command.Parameters.AddWithValue("@ComponentId", ddPackingItems.SelectedValue);
        command.Parameters.AddWithValue("@ComponentName", ddPackingItems.SelectedItem.Text);
        command.Parameters.AddWithValue("@ConsumptionQuantity", totalBagRequired);
        command.Parameters.AddWithValue("@Rate", packingMaterialsRate);
        command.Parameters.AddWithValue("@ConsumedAmount", packingMaterialsConsumedAmount);
        //command.Parameters.AddWithValue("@Remarks", consumedAmount);
        command.Parameters.AddWithValue("@EntryBy", lName);
        command.Parameters.AddWithValue("@EntryDate", Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"));

        command.Connection.Open();
        command.ExecuteNonQuery();
        command.Connection.Close();

        string maxOrderDetailsId = SQLQuery.ReturnString("SELECT MAX(Id) FROM OrderDetails");
        decimal profitAmount = Convert.ToDecimal(txtQty.Text) - totalComponentAmount;
        SQLQuery.ExecNonQry("UPDATE OrderDetails SET ProfitAmount = '" + profitAmount + "' WHERE Id = '" + maxOrderDetailsId + "'");
    }

    protected decimal InsertCostOfGoodsSoldData(int salesDetailsMasterId, string orderID)
    {
        string lName = Page.User.Identity.Name;
        decimal grossTotalAmount = 0;
        foreach (GridViewRow row in ConsumedItemGrid.Rows)
        {
            Label labelComponentId = row.FindControl("labelComponentId") as Label;
            Label labelComponentName = row.FindControl("labelComponentName") as Label;
            TextBox txtDeliveredQuantity = row.FindControl("txtQty") as TextBox;
            ...
            decimal totalQuantity = Convert.ToDecimal(txtDeliveredQuantity.Text) + Convert.ToDecimal(txtExcessQuantity.Text);
            decimal consumedAmount = totalQuantity * Convert.ToDecimal(txtRate.Text);
            grossTotalAmount += consumedAmount;
            TextBox txtRemarks = row.FindControl("txtRemarks") as TextBox;
            string temp = SQLQuery.ReturnString("SELECT InKg FROM tblFifo WHERE ItemCode = '" + labelComponentId.Text +
                                                "' AND (InType = 'SemiFinished' OR InType = 'Poly Production Output')");
            decimal unitWeight = Convert.ToDecimal(temp);
            ...
            cmd.Parameters.AddWithValue("@SalesDetailsMasterId", salesDetailsMasterId);
            cmd.Parameters.AddWithValue("@ComponentId", Convert.ToDecimal(labelComponentId.Text));
            cmd.Parameters.AddWithValue("@ComponentName", labelComponentName.Text);
            cmd.Parameters.AddWithValue("@DeliveredQuantity", Convert.ToDecimal(txtDeliveredQuantity.Text));
            cmd.Parameters.AddWithValue("@Rate", txtRate.Text);
            cmd.Parameters.AddWithValue("@ExcessQuantity", txtExcessQuantity.Text);
            cmd.Parameters.AddWithValue("@TotalQuantity", totalQuantity);
            cmd.Parameters.AddWithValue("@TotalWeight", unitWeight * totalQuantity);
            cmd.Parameters.AddWithValue("@ConsumedAmount", consumedAmount);
            ...
            Stock.Inventory.FifoInsert(labelComponentId.Text, "RawMaterialsConsumption", ddCustomer.SelectedValue, "", "", "", Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"), "", "", 0, totalQuantity, totalQuantity, "", "", Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"), "RawMaterialsConsumption", orderID, "");
    ...
    private void ExecuteUpdate()
    {
        ...
        SQLQuery.ExecNonQry("DELETE BOQConsumption WHERE ProductId = '" + ddProductName.SelectedValue + "' AND SalesDetailsId = '' AND InvoiceNo=''");
        ...
        decimal profitAmount = Convert.ToDecimal(txtQty.Text) - totalComponentAmount;
        SQLQuery.ExecNonQry("UPDATE OrderDetails SET ProfitAmount = '" + profitAmount + "' WHERE Id = '" + lblOrderID.Text + "'");
    }
    ...
    private void BindConsumedItemGrid()
    {
        string lName = Page.User.Identity.Name;
        //SqlCommand command = new SqlCommand("SELECT Id, InvoiceNo, CompanyId, CompanyName, ProductId, ProductName, ComponentId, ComponentName, ConsumptionQuantity, Rate, ConsumedAmount, '0' AS ExcessQuantity, '' AS Remarks, EntryBy, EntryDate FROM BOQConsumption WHERE InvoiceNo = '' AND EntryBy='" + lName + "' ORDER BY Id", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        SqlCommand command = new SqlCommand("SELECT ComponentId, ComponentName, SUM(ConsumptionQuantity) AS ConsumptionQuantity, Rate, SUM(ConsumedAmount) AS ConsumedAmount, '0' AS ExcessQuantity, '' AS Remarks FROM BOQConsumption WHERE InvoiceNo = '' AND EntryBy='" + lName + "' GROUP BY ComponentId, ComponentName, Rate ", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        command.Connection.Open();
        ConsumedItemGrid.EmptyDataText = "No items to view...";
        ConsumedItemGrid.DataSource = command.ExecuteReader();
        ConsumedItemGrid.DataBind();
        command.Connection.Close();
    }

    private void BindComponentGrid()
    {
        string lName = Page.User.Identity.Name.ToString();
        //SqlCommand cmd = new SqlCommand("SELECT Id, ComponentId, ComponentName FROM Components", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        SqlCommand cmd = new SqlCommand("SELECT TOP(5) Id, ItemCode, '1' AS Quantity FROM tblFifo", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));

        cmd.Connection.Open();
        ComponentGrid.EmptyDataText = "No items to view...";
        ComponentGrid.DataSource = cmd.ExecuteReader();
        ComponentGrid.DataBind();
        cmd.Connection.Close();
    }
    ...
    protected void ConsumedItemGrid_OnRowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            int index = Convert.ToInt32(e.RowIndex);
            Label labelEntryId = ConsumedItemGrid.Rows[index].FindControl("lblEntryId") as Label;

            SqlCommand cmd7 = new SqlCommand("DELETE BOQConsumption WHERE Id=" + labelEntryId.Text + " AND InvoiceNo = ''", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
            cmd7.Connection.Open();
            cmd7.ExecuteNonQuery();
            cmd7.Connection.Close();
            ...
            Notify("Comsumption data has been deleted from this order ...", "warn", lblMsg2);
            //lblMsg2.Attributes.Add("class", "xerp_warning");
            //lblMsg2.Text = "Item deleted from order ...";
        }
        catch (Exception ex)
    ...
    protected void ConsumedItemGrid_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        try
        {
            int index = Convert.ToInt32(ConsumedItemGrid.SelectedIndex);
            Label labelEntryId = ConsumedItemGrid.Rows[index].FindControl("lblEntryId") as Label;

            EditMode(labelEntryId.Text);
            btnAdd.Text = "Update";
