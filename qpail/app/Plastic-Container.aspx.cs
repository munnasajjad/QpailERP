...
public partial class app_Plastic_Container : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            string sum = RunQuery.SQLQuery.ReturnString("SELECT ISNULL(ActProduction,0) FROM PrdPlasticCont Where ProductionID='' AND EntryBy='" + lblUser.Text + "' ");
            if (sum != "")
            {
                CalcConsumption();
            }
    ...
    private void LoadPendingEntries()
    {
        ...
        {
            string lName = Page.User.Identity.Name.ToString();
            string isExist = RunQuery.SQLQuery.ReturnString("Select pid from PrdPlasticCont where ProductionID='' AND EntryBy='" + lName + "'");
            if (isExist == "")
            {
                SqlCommand cmd2 = new SqlCommand("INSERT INTO PrdPlasticCont (ProductionID, EntryBy) VALUES (@ProductionID, @EntryBy)",
            ...
            else
            {
                SqlCommand cmd = new SqlCommand(
                        @"SELECT Date, Shift, MachineNo, LineNumber, Purpose, CustomerID, Brand, PackSize, Operation, OperatorID, CycleTime, 
                         ...
                         Category, ItemID, Color, FinalProduction, WasteWeight, ReUsableItem, Remarks FROM [PrdPlasticCont] WHERE ProductionID=''  AND EntryBy='" + lName + "' ",
                        new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
                cmd.Connection.Open();
    ...
    {
        string prdId = RunQuery.SQLQuery.ReturnString("Select 'PRD-PC-'+ CONVERT(varchar, (ISNULL (max(pid),0)+1001 )) from PrdPlasticCont");
        lblPrId.Text = prdId;
    ...
    {
        string lName = Page.User.Identity.Name.ToString();
        RunQuery.SQLQuery.ExecNonQry(@"Update PrdPlasticCont SET Date='" +
        ...
        RunQuery.SQLQuery.ExecNonQry(@"Update PrdPlasticCont SET CycleTime='" +
                                     txtCycleTime.Text +
                                     "', CavityPcs='" +
                                     txtCavity.Text +
        ...
        RunQuery.SQLQuery.ExecNonQry(@"Update PrdPlasticCont SET ItemSubGroup='" +
    ...
    private void StockEntry()
    {
        ...
        RunQuery.SQLQuery.ExecNonQry("UPDATE PrdPlasticContDetails SET PrdnID='" + lblPrId.Text + "' WHERE PrdnID=''  AND EntryBy='" + lblUser.Text + "'  ");
        RunQuery.SQLQuery.ExecNonQry("UPDATE PrdPlasticCont SET ProductionID='" + lblPrId.Text + "' WHERE ProductionID=''  AND EntryBy='" + lblUser.Text + "' ");

        //Input Item Stock-out
        DataTable citydt = RunQuery.SQLQuery.ReturnDataTable("Select Purpose, SizeId, BrandID, ProductID, ProductName, TotalWeight, ConsumedWeight from PrdPlasticContDetails where PrdnID='" + lblPrId.Text + "'");
        ...
        foreach (DataRow citydr in citydt.Rows)
        {
            ...
            try
            {
                //string outId = SQLQuery.ReturnString("SELECT pid FROM PrdPlasticCont WHERE ProductionID='" + lblPrId.Text + "'");
    ...
    {
        SqlCommand cmd3 = new SqlCommand("INSERT INTO PrdPlasticContDetails (PrdnID, Department, Purpose, SizeId, BrandID, ItemGroup, SubGroup, Grade, Category, ProductID, ProductName, TotalWeight, ConsumedWeight, EntryBy)" +
    ...
    {
        RunQuery.SQLQuery.ExecNonQry(@"UPDATE [dbo].[PrdPlasticContDetails]   SET 
    ...
    {
        SqlCommand cmd = new SqlCommand("SELECT Department, Purpose, SizeId, BrandID, ProductID, TotalWeight, ConsumedWeight, ItemGroup, SubGroup, Grade, Category FROM [PrdPlasticContDetails] WHERE Id='" + entryID + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Connection.Open();
        SqlDataReader dr = cmd.ExecuteReader();
        if (dr.Read())
        {
    ...
    private void CalcConsumption()
    {
        try
        {
            ...
            decimal ttlConsumed = Convert.ToDecimal(RunQuery.SQLQuery.ReturnString("SELECT ISNULL(SUM(TotalWeight),0) FROM PrdPlasticContDetails Where PrdnID='" + prdnNo + "' AND EntryBy='"+User.Identity.Name+"' "));
            decimal actPrdn = Convert.ToDecimal(RunQuery.SQLQuery.ReturnString("SELECT ISNULL(ActualWeight,0) FROM PrdPlasticCont Where ProductionID='" + prdnNo + "' AND EntryBy='" + User.Identity.Name + "' "));
            if (txtActualWeight.Text != "")
            {
                actPrdn = Convert.ToDecimal(txtActualWeight.Text);
            }
            DataTable prdData = RunQuery.SQLQuery.ReturnDataTable("Select Id, TotalWeight FROM PrdPlasticContDetails Where PrdnID='" + prdnNo + "' AND EntryBy='" + User.Identity.Name + "'  ");
            ...
            foreach (DataRow row in prdData.Rows)
            {
                ...
                RunQuery.SQLQuery.ExecNonQry("UPDATE PrdPlasticContDetails SET ConsumedWeight='" + consumption + "' WHERE id='" + entryID + "'");
                Bind2ndGrid();
            }
        }
        catch (Exception ex)
    ...
    private void Bind2ndGrid()
    {
        ...
        query = @"SELECT Id, (Select Purpose from Purpose where pid = a.Purpose) AS[Purpose2], 
                                    ...
                                    ProductID, ProductName, TotalWeight, ConsumedWeight FROM [PrdPlasticContDetails] a
                               WHERE Id=(SELECT MAX(pid) FROM PrdPlasticCont WHERE MachineNo='"+ddMachine.SelectedValue+"')" + query;
...
FROM            PrdPlasticCont WHERE (pid = '" + itemToEdit + "')");
    ...
    protected void GridView1_OnRowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            string isExist = RunQuery.SQLQuery.ReturnString("Select ProductionID FROM PrdPlasticCont WHERE pid='" + lblItemCode.Text + "'");

            SQLQuery.ExecNonQry("Delete PrdPlasticCont where ProductionID ='" + isExist + "'   ");
            SQLQuery.ExecNonQry("Delete PrdPlasticContDetails where PrdnID='" + isExist + "'   ");
