...
public partial class app_StockHeadLedger : System.Web.UI.Page
{
    ...
    //    if (isLiability == "02")
    //    {
    //        decimal opBal = Convert.ToDecimal(RunQuery.SQLQuery.ReturnString("SELECT  isnull(sum(OpBalCr),0) - isnull(sum(OpBalDr),0)  FROM [HeadSetup] WHERE ([AccountsHeadID] = '" + ddParties.SelectedValue + "')"));
    //        decimal preBal = Convert.ToDecimal(RunQuery.SQLQuery.ReturnString("SELECT  isnull(sum(VoucherCR),0) - isnull(sum(VoucherDR),0) FROM [VoucherDetails] WHERE ([AccountsHeadID] = '" + ddParties.SelectedValue + "') and   EntryDate < '" + Convert.ToDateTime(txtDateFrom.Text).ToString("yyyy-MM-dd") + "'  and ISApproved='A'"));
    ...
    //        //get closing balance
    //        currBal = Convert.ToDecimal(RunQuery.SQLQuery.ReturnString("SELECT  isnull(sum(VoucherCR),0) - isnull(sum(VoucherDR),0) FROM [VoucherDetails] WHERE ([AccountsHeadID] = '" + ddParties.SelectedValue + "') and   EntryDate < '" + Convert.ToDateTime(txtDateTo.Text).AddDays(+1).ToString("yyyy-MM-dd") + "'  and ISApproved='A'"));
    ...
    private void LoadStockHead()
    {
        //if (chkFiniGoods.Checked)
        //{
        //    ddParties.Items.Clear();
        //    SQLQuery.PopulateDropDown("SELECT pid, ProductName +' > '+(SELECT Company FROM Party WHERE PartyID=FinishedProducts.CompanyID) AS ProductName FROM FinishedProducts", ddParties, "pid", "ProductName");
        //}
        //else
        //{
        //    ddParties.Items.Clear();
        //    SQLQuery.PopulateDropDown("Select ProductID, ItemName from Products WHERE CategoryID IN (SELECT CategoryID FROM Categories WHERE GradeID IN (SELECT GradeID FROM ItemGrade WHERE CategoryID IN (SELECT CategoryID FROM ItemSubGroup WHERE GroupID = '11')))", ddParties, "ProductID", "ItemName");
        //}
    }
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            if (Convert.ToDateTime(txtDateFrom.Text) <= Convert.ToDateTime(txtDateTo.Text))
            {
                ...
                //                  "XerpReports/FormAccHeadQtyLedger.aspx?item=" + ddParties.SelectedValue + "&dt1=" + dt1 +
                //                  "&dt2=" + dt2;
                //    if1.Attributes.Add("src", urlx);
                //}
                //else
                //{
                    string urlx = SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") + "XerpReports/StockHeadLedger.aspx?item=" + ddParties.SelectedValue + "&dt1=" + dt1 + "&dt2=" + dt2 + "&chkValue=" + chkFiniGoods.Checked;
                    if1.Attributes.Add("src", urlx);
                //}
    ...
    protected void chkFiniGoods_OnCheckedChanged(object sender, EventArgs e)
    {
        LoadStockHead();
    }
}