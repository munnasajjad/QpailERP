...
public partial class app_Sale_Type_Year : System.Web.UI.Page
{
    ...
    private int CalcQty(string customerId, string dtFrom, string dtTo)
    {
        ...
        {
            qty = Convert.ToInt32(Convert.ToDecimal(SQLQuery.ReturnString("Select ISNULL(SUM(" + colName + "),0)  FROM [SaleDetails] WHERE ProductID IN (Select ProductID from Products where CategoryID IN (SELECT CategoryID from Categories where GradeID='" + ddGrade.SelectedValue + "')) AND   InvNo IN (SELECT InvNo FROM Sales WHERE (CustomerID = '" + customerId + "') AND (DeliveryDate >= '" + dtFrom + "') AND (DeliveryDate <= '" + dtTo + "'))")));
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string urlx = SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") + "XerpReports/FormSale-Type-Year.aspx?igrade=" + ddGrade.SelectedValue + "&iRType=" + ddType.SelectedValue + "&ichk=" + chk + "&iyear=" + txtYear.Text;
    ...
    {
        exportExcel(LoadGridData(), "Year-Sales-by-Grade-and-Company-Report");
    }
