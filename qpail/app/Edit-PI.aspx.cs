...
public partial class app_Edit_PI : System.Web.UI.Page
{
    ...
    private void BindItemGrid()
    {
        ...
        cmd7 = new SqlCommand("SELECT PiDate, PiTerms FROM [PI] WHERE PiNo='" + OrderID + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd7.Connection.Open();
        ...
        SqlCommand cmd2 = new SqlCommand("SELECT PiDate, LcNo, LcDate, LcExpDate, IssuingBank, IssueingBankBranch, AdvisingBankAccount, PiTerms FROM [PI] WHERE OrderID='" + OrderID + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    private void UpdateOrder()
    {
        ...
        RunQuery.SQLQuery.ExecNonQry("Update PI set PiDate='" + orderDate + "', PiTerms='" + txtTerms.Text + "', LcEntryBy='" + lName + "', LcEntryDate='" + DateTime.Now.ToString("yyyy-MM-dd") + "' Where PiNo='" + ddOrders.SelectedValue + "'");
        RunQuery.SQLQuery.ExecNonQry("UPDATE PO_Images SET PONo='" + ddOrders.SelectedValue + "', ImgDescription='PI' WHERE EntryBy='" + lName + "' AND (PONo='' OR PONo='" + ddOrders.SelectedValue + "')");

        //string PIExist = RunQuery.SQLQuery.ReturnString("Select OrderID FROM OrdersLC where OrderID='" + ddOrders.SelectedValue + "'");

        //SqlCommand cmd = new SqlCommand("UPDATE OrderDetails SET OrderID='" + txtEditPI.Text + "'  WHERE OrderID='" + ddOrders.SelectedValue + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        //cmd.Connection.Open();
        //cmd.ExecuteNonQuery();
        //cmd.Connection.Close();
