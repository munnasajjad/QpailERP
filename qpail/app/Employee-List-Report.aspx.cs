...
public partial class Operator_Employee_List_Report : System.Web.UI.Page
{
    ...
    private void LoadGridData()
    {
        ...
        DataTable dt = RunQuery.SQLQuery.ReturnDataTable("SELECT [InvNo], [ProductName], [Quantity], [UnitType], [UnitCost], [ItemTotal], [UnitWeight], [TotalWeight], [VAT], NetAmount, [EntryDate], [ReturnQty] FROM [SaleDetails] WHERE ([SizeId] = '" + sizeId + "') and ([ProductID] = '" + pId + "') and([BrandID] = '" + brandId + "') and EntryDate >= '" + dtFrom.ToString("yyyy-MM-dd") + "' AND EntryDate <= '" + dtTo.ToString("yyyy-MM-dd") + "' ORDER BY Id DESC ");
        if (CheckBox1.Checked)
        {
            dt = RunQuery.SQLQuery.ReturnDataTable("SELECT [InvNo], [ProductName], [Quantity], [UnitType], [UnitCost], [ItemTotal], [UnitWeight], [TotalWeight], [VAT], NetAmount, [EntryDate], [ReturnQty] FROM [SaleDetails] WHERE InvNo IN (Select InvNo from sales where ProductID IN (Select ProductID from Products where CategoryID IN (SELECT CategoryID from Categories where GradeID='" + ddGrade.SelectedValue + "')) AND CustomerID = '" + ddCustomer.SelectedValue + "' and DeliveryDate >= '" + dtFrom.ToString("yyyy-MM-dd") + "' AND DeliveryDate <= '" + dtTo.ToString("yyyy-MM-dd") + "') ORDER BY Id DESC ");
        }
        ...
        foreach (DataRow dr in dt.Rows)
        {
            ...
            dr1["VAT"] = Convert.ToString(dr["VAT"]);
            dr1["TotalAmt"] = Convert.ToString(dr["NetAmount"]);
            dt1.Rows.Add(dr1);

            total2 += Convert.ToDecimal(Convert.ToString(dr["ItemTotal"]));
            vat2 += Convert.ToDecimal(Convert.ToString(dr["VAT"]));
            qty2 += Convert.ToDecimal(Convert.ToString(dr["Quantity"]));
            TotalSales2 += Convert.ToDecimal(Convert.ToString(dr["NetAmount"]));
