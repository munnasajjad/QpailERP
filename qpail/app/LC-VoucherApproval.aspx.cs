...
public partial class app_LC_VoucherApproval : System.Web.UI.Page
{
    ...
    private void search()
    {
        try
        {
            ...
            DataSet ds = SQLQuery.ReturnDataSet(@"SELECT        LC.LCNo, VM.VID, VM.VoucherNo, VM.VoucherDate, VM.VoucherDescription, VM.ParticularID, VM.VoucherEntryBy, VM.VoucherEntryDate, VM.Voucherpost, VM.VoucherPostby, VM.Voucherpostdate, VM.VoucherReferenceNo, 
                         VM.VoucherRefType, VM.VoucherAmount, VM.projectName, VM.VoucherEntryTime
FROM            VoucherMaster AS VM INNER JOIN
                         LC ON VM.VoucherReferenceNo = LC.sl
WHERE        (VM.Voucherpost = 'c') AND (VM.VoucherReferenceNo = '"+ddCustomer.SelectedValue+"') ORDER BY LC.LCNo desc");
