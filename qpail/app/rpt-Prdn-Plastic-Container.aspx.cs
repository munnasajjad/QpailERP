...
public partial class app_rpt_Prdn_Plastic_Container : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        string url = SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") +
            "XerpReports/FormPlasticContainerHistory.aspx?purpose="+ddPurpose.SelectedValue+"&item="+ddItemName.SelectedValue + "&cust=" + ddCustomer.SelectedValue + "&pkSize=" + ddSize.SelectedValue + "&pCcolor=" + ddColor.SelectedValue + "&dateFrom=" + dateFrom + "&dateTo=" + dateTo;
    ...
    private DataTable BindItemGrid()
    {
        ...
        try
        {
            ...
            {
                item = " AND (ItemID = '" + ddItemName.SelectedValue + "')  ";
            }
            ...
            {
                cust = " AND CustomerID='" + ddCustomer.SelectedValue + "' ";
            }

            string pkSize = " ";
            if (ddSize.SelectedValue != "--- all ---")
            {
                pkSize = " AND PackSize='" + ddSize.SelectedValue + "' ";
            }
            //string Color = " ";
            //if (ddColor.SelectedValue != "--- all ---")
            //{
            //    Color = " AND PackSize='" + ddColor.SelectedValue + "' ";
            //}

            string query = " FROM PrdPlasticCont a where pid<>0 " + item + dateFrom + dateTo + cust + pkSize;
