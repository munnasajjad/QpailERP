...
public partial class app_RequisitionApproval : System.Web.UI.Page
{
    ...
    private void InsertData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO StockinDetailsRaw (StockType, purpose, ItemType, OrderID, ProductID, ProductName, Manufacturer, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor, GodownID, LocationID, QtyBalance, DeliveredQty, Spec) " +
    ...
    private void BindItemGrid()
    {
        ...
        SqlCommand cmd = new SqlCommand(@"SELECT Id, ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, QtyBalance, DeliveredQty, UnitWeight, Manufacturer
                                            FROM StockinDetailsRaw WHERE GodownID='" + ddFromGodown.SelectedValue + "' AND LocationID = '"+ddFromLocation.SelectedValue+"' AND EntryBy=@EntryBy AND OrderID='' AND StockType='Raw'  ORDER BY Id DESC", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    protected void ItemGrid_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        //ltrQty.Text = SQLQuery.ReturnString("Select ISNULL(SUM(Quantity),0) from StockinDetailsRaw where OrderID='" + ddIssueItem.SelectedValue + "'") + "pcs, " +
        //    SQLQuery.ReturnString("Select ISNULL(SUM(UnitWeight),0) from StockinDetailsRaw where OrderID='" + ddIssueItem.SelectedValue + "'") + "kg";
    ...
    private void ExecuteInsert()
    {
        ...
        {
            SqlCommand cmd2 = new SqlCommand("INSERT INTO Stockin (OrderID, OrderDate, GodownID, GodownName, LocationID, LocationName, ToGodownID, ToLocationID, Remarks, TotalQty, EntryBy, ProjectId,ApprovalStatus)" +
                    " VALUES (@OrderID, @OrderDate, @GodownID, @GodownName, @LocationID, @LocationName, @ToGodownID, @ToLocationID, @Remarks, @TotalQty, @EntryBy, @ProjectId,@ApprovalStatus)",
            ...
            cmd2.Parameters.AddWithValue("@LocationName", ddFromLocation.SelectedValue);
        ...
        //Sock-in
        else if (chkApprove.Checked == true)
        {
            string entryId = "";
            foreach (GridViewRow row in GVrpt.Rows)
            {
                Label lblEntryId = row.FindControl("txtID") as Label;
                entryId = lblEntryId.Text;
                ...
                string ProductName = SQLQuery.ReturnString("SELECT ProductName FROM StockinDetailsRaw WHERE (Id='" + lblEntryId.Text +
                                          "')");
                string detail = SQLQuery.ReturnString("SELECT Manufacturer FROM StockinDetailsRaw WHERE (Id='" + lblEntryId.Text +
                                          "')");
                ...
                TextBox lbliQty = row.FindControl("txtQty") as TextBox;
                TextBox lblUnitWeight = row.FindControl("txtWeight") as TextBox;
                Label lblQtyBalance = row.FindControl("lblQtyBalance") as Label;

                //int inQty = Convert.ToInt32(lbliQty.Text);
                decimal inQty = Convert.ToDecimal(lbliQty.Text);
                decimal inWeight = Convert.ToDecimal(lblUnitWeight.Text);
                Stock.Inventory.SaveToStock(purpose, orderNo, detail, lblEntryId.Text, sizeId, customer, brandID, color,
                    spec, productID, ProductName, itemType, ddGodown.SelectedValue, ddLocation.SelectedValue,
                    ddGroup.SelectedValue, inQty, 0, price, inWeight, 0, "", "Stock-in", "Transfer", "Store", lName,
                    Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"), detail);
                Stock.Inventory.SaveToStock(purpose, orderNo, detail, lblEntryId.Text, sizeId, customer, brandID, color,
                    spec, productID, ProductName, itemType, ddFromGodown.SelectedValue, ddFromLocation.SelectedValue, ddGroup.SelectedValue, 0,
                    inQty, price, 0, inWeight, "", "Stock-out", "Transfer", "Store", lName,
                    Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"), detail);

            }
            //try
            //{
            //    //string entryId = SQLQuery.ReturnString("SELECT Id FROM StockinDetailsRaw where Id='" + ddIssueItem.SelectedValue + "'");
            //    string productID = SQLQuery.ReturnString("SELECT ProductID FROM StockinDetailsRaw WHERE Id='" + entryId + "'");
            //    decimal price = Convert.ToDecimal(SQLQuery.ReturnString("SELECT UnitCost FROM StockinDetailsRaw WHERE Id='" + entryId + "'"));
            //    decimal inWeight = Convert.ToDecimal(SQLQuery.ReturnString("SELECT UnitWeight FROM StockinDetailsRaw WHERE Id='" + entryId + "'"));
            //    //  decimal inWeight = Convert.ToDecimal(txtWeight.Text);

            //    Stock.Inventory.FifoInsert(productID, ddSize.SelectedValue, ddColor.SelectedValue, Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"), "Requisition Approve", entryId, price.ToString(), inWeight, DateTime.Now.ToString("yyyy-MM-dd"), "", "", "0");
            ...
            SqlCommand cmd = new SqlCommand("UPDATE Stockin SET ApprovalStatus='Approved',UpdateDate='" + DateTime.Now.ToString("MM/dd/yyyy") + "' WHERE  Stockinl='" + ddIssueItem.SelectedValue + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
            cmd.Parameters.AddWithValue("@EntryBy", lName);
            cmd.Connection.Open();
            cmd.ExecuteNonQuery();
            cmd.Connection.Close();
    ...
    {
        SqlCommand cmd = new SqlCommand(@"SELECT ItemType, ProductID, SizeId, Customer, BrandID, Color, Quantity, UnitWeight, GodownID, Manufacturer, UnitCost FROM [StockinDetailsRaw] WHERE Id='" + entryID + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Connection.Open();
    ...
    protected void ddIssueItem_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        LoadData();
        //BindItemGrid();
        loadGrid();
    ...
    private void loadGrid()
    {
        ...
        if (!string.IsNullOrEmpty(ddIssueItem.SelectedValue))
        {


            /*DataTable dt = SQLQuery.ReturnDataTable(@"SELECT Id,ProductName,Manufacturer,Quantity,UnitWeight FROM StockinDetailsRaw INNER JOIN Stockin on StockinDetailsRaw.Id=Stockin.Stockinl  Where Id='" + ddIssueItem.SelectedValue + "'");*/
            /**/
            DataTable dt = SQLQuery.ReturnDataTable(@"SELECT StockinDetailsRaw.Id, StockinDetailsRaw.ProductName, StockinDetailsRaw.Manufacturer, StockinDetailsRaw.Quantity, StockinDetailsRaw.UnitWeight
            FROM StockinDetailsRaw INNER JOIN Stockin ON StockinDetailsRaw.OrderID = Stockin.OrderID WHERE (Stockin.Stockinl = '" + ddIssueItem.SelectedValue + "')");
            GVrpt.DataSource = dt;
            GVrpt.DataBind();
    ...
    private void LoadData()
    {
        ...
        // string strQuery = (@"SELECT  OrderDate, GodownID, GodownName, LocationID, LocationName, Remarks, TotalQty, EntryBy, ProjectId FROM Stockin WHERE OrderID='"+ddIssueItem.SelectedValue+"'");
        string requisitionDate = SQLQuery.ReturnString(@"SELECT OrderDate FROM Stockin WHERE Stockinl='" + ddIssueItem.SelectedValue + "' ");
        string fromGodown = SQLQuery.ReturnString(@"SELECT GodownID FROM Stockin WHERE Stockinl='" + ddIssueItem.SelectedValue + "' ");
        string fromLocation = SQLQuery.ReturnString(@"SELECT LocationID FROM Stockin WHERE Stockinl='" + ddIssueItem.SelectedValue + "' ");
        string toGodown = SQLQuery.ReturnString(@"SELECT ToGodownID FROM Stockin WHERE Stockinl='" + ddIssueItem.SelectedValue + "' ");
        string toLocation = SQLQuery.ReturnString(@"SELECT ToLocationID FROM Stockin WHERE Stockinl='" + ddIssueItem.SelectedValue + "' ");
        string remarks = SQLQuery.ReturnString(@"SELECT Remarks FROM Stockin WHERE Stockinl='" + ddIssueItem.SelectedValue + "' ");

        //  string id = SQLQuery.ReturnString(@"SELECT Id FROM StockinDetailsRaw WHERE OrderID='" + ddIssueItem.SelectedValue + "'");
        try
        {
            string ttlQty = SQLQuery.ReturnString(@"SELECT ISNULL(SUM(Quantity),0) FROM StockinDetailsRaw WHERE Id='" + ddIssueItem.SelectedValue + "'") + "pcs, " +
       SQLQuery.ReturnString(@"SELECT ISNULL(SUM(UnitWeight),0) FROM StockinDetailsRaw WHERE Id='" + ddIssueItem.SelectedValue + "'") + "kg";
            ltrQty.Text = ttlQty;
        }
        catch (Exception exc)
        {
            Notify(exc.Message.ToString(), "error", lblMsg);

        }

        txtDate.Text = Convert.ToDateTime(requisitionDate).ToString("dd/MM/yyyy");
        ddFromGodown.SelectedValue = fromGodown;
        ddFromLocation.DataBind();
        ddFromLocation.SelectedValue = fromLocation;
        ddGodown.SelectedValue = toGodown;
        ddLocation.DataBind();
        ddLocation.SelectedValue = toLocation;
        txtRemarks.Text = remarks;
    }

    protected void GVrpt_OnRowDataBound(object sender, GridViewRowEventArgs e)
    {

        //ltrQty.Text = SQLQuery.ReturnString("Select ISNULL(SUM(Quantity),0) from StockinDetailsRaw where OrderID='" + ddIssueItem.SelectedValue + "' AND GodownID='" + ddGodown.SelectedValue + "'") + "pcs, " +
        //    SQLQuery.ReturnString("Select ISNULL(SUM(UnitWeight),0) from StockinDetailsRaw where OrderID='" + ddIssueItem.SelectedValue + "' AND GodownID='" + ddGodown.SelectedValue + "'") + "kg";

        // ltrQty.Text = Qty.ToString();// Show gridview Footer

        //if (e.Row.RowType == DataControlRowType.Header)
        //{
        //    e.Row.Cells[2].Text = "Item Name";
        //    e.Row.Cells[1].Text = "Item Details";
        //    if (lblGroup.Text == "1")
        //    {
        //        e.Row.Cells[2].Text = "Color Name";
        //        e.Row.Cells[1].Text = "Composition";
        //    }
        //    else if (lblGroup.Text == "5")
        //    {
        //        e.Row.Cells[2].Text = "Parts Name";
        //    }
        //}
    }


    protected void GVrpt_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        //ltrQty.Text = SQLQuery.ReturnString("Select ISNULL(SUM(Quantity),0) from StockinDetailsRaw where OrderID='" + ddIssueItem.SelectedValue + "' AND GodownID='" + ddGodown.SelectedValue + "'") + "pcs, " +
        //  SQLQuery.ReturnString("Select ISNULL(SUM(UnitWeight),0) from StockinDetailsRaw where OrderID='" + ddIssueItem.SelectedValue + "' AND GodownID='" + ddGodown.SelectedValue + "'") + "kg";
