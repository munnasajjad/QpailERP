...
public partial class Cells_Date_Wise_Orders : System.Web.UI.Page
{
    ...
    {
        SqlCommand cmd2 = new SqlCommand("SELECT [InvNo] as [Inv.No.], [PuchaseDate] as [Entry Time], ReceiverBranchID as [Delivery Branch], [PurchaseFrom] as [Sender], [SenderMobile] as [S.Mobile#], [SalesTo] as [Receiver], ReceiverMobile as [R.Mobile#], PurchaseTaka as [Transfer Amount], Collected, SmsStatus as [SMS]  FROM [Orders] WHERE ([SenderBranchID] = @HeadName) and   PuchaseDate >= @DateFrom AND PuchaseDate <= @DateTo ORDER BY [PID] DESC", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
