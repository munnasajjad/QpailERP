...
public partial class app_Sale_Item_byBrand_Report : System.Web.UI.Page
{
...
/*
    private DataTable LoadGridData()
    {
        ...
        if (!CheckBox2.Checked) // Not All Customers
        {
        ...
        DataTable dt = RunQuery.SQLQuery.ReturnDataTable("SELECT [InvNo], [ProductName], [Quantity], [UnitType], [UnitCost], [ItemTotal], [UnitWeight], [TotalWeight], [VAT], NetAmount, [EntryDate], [ReturnQty] FROM [SaleDetails] WHERE ID<>0 " + query + " ORDER BY Id DESC ");
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            if (Convert.ToDateTime(txtDateFrom.Text) <= Convert.ToDateTime(txtDateTo.Text))
            {
                ...
                string urlx = SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") + "XerpReports/FormSalesbyBrand.aspx?size=" + ddItem.SelectedValue + "&brand=" + ddBrand.SelectedValue + "&iGrade=" + ddGrade.SelectedValue + "&chk=" + chk + "&chk2=" + chk2 + "&cust=" + ddCustomer.SelectedValue + "&dt1=" + dtFrom + "&dt2=" + dtTo;
