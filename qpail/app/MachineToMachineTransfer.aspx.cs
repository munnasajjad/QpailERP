...
public partial class app_MachineToMachineTransfer : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            if (Convert.ToDecimal(txtCurrentQty.Text) >= Convert.ToDecimal(txtQty.Text) && Convert.ToDecimal(txtCurrentKg.Text) >= Convert.ToDecimal(txtWeight.Text))
            {
                if (btnAdd.Text == "Add")
                {
                    string lName = Page.User.Identity.Name.ToString();
                    ...
                    SqlCommand cmde = new SqlCommand("SELECT ProductName FROM MachineStockTransferDetails WHERE  ProductID ='" + ddProduct.SelectedValue + "' AND  OrderID ='' AND EntryBy ='" + lName + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
                    ...
                    //   lblMsg2.Text = "ERROR: You must edit quantity for Transfer!";
                    //}
                    //}
                    //else
                    //{
                ...
                {
                    //ExecuteUpdate();
                    SQLQuery.ExecNonQry("Delete MachineStockTransferDetails where (id ='" + lblOrderID.Text + "')");
                    InsertData();
                    btnAdd.Text = "Add";
            ...
            {
                Notify("ERROR: Item is not available into stock!", "warn", lblMsg);
            }
        }
        catch (Exception ex)
        {
    ...
    private void InsertData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO MachineStockTransferDetails (StockType, purpose, ItemType, OrderID, ProductID, ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor,  MachineID, QtyBalance, DeliveredQty, Spec) " +
                                         "VALUES ('Raw', '" + ddPurpose.SelectedValue + "', '" + ddType.SelectedValue + "', @OrderID, @ProductID, @ProductName, @UnitCost, @Quantity, @UnitWeight, '" + ltrUnit.Text + "', @ItemTotal, @TotalWeight, @EntryBy, @CompanyFor, '" + ddMachineFrom.SelectedValue + "', '" + balance + "', '" + ltrCQty.Text + "', '" + spec + "'  )", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));

        if (ddType.SelectedValue == "Printed Sheet")
        {
            cmd2 = new SqlCommand(@"INSERT INTO MachineStockTransferDetails (StockType, purpose, ItemType, OrderID, SizeId, ProductID, Customer, BrandID, ProductName, Color, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor,  MachineID, QtyBalance, DeliveredQty)" +
                                        " VALUES ('Raw', '" + ddPurpose.SelectedValue + "', '" + ddType.SelectedValue + "', @OrderID, @SizeId, @ProductID, '" + ddCustomer.SelectedValue + "', @BrandID, @ProductName, '" + ddColor.SelectedValue + "', @UnitCost, @Quantity, @UnitWeight, '" + ltrUnit.Text + "', @ItemTotal, @TotalWeight, @EntryBy, @CompanyFor, '" + ddMachineFrom.SelectedValue + "', '" + balance + "', '" + ltrCQty.Text + "'  )", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        }
    ...
    //    int balance = Convert.ToInt32(txtQty.Text) - Convert.ToInt32(ltrCQty.Text);
    //    SqlCommand cmd2 = new SqlCommand("UPDATE StockTransferDetails SET SizeId='" + ddGroup.SelectedValue + "', ProductID='" + ddProduct.SelectedValue + "', BrandID='" + ddSubGroup.SelectedValue + "'," +
    ...
    {
        string lName = Page.User.Identity.Name.ToString();
        SqlCommand cmd = new SqlCommand(@"SELECT  Id, (SELECT  [Purpose] FROM [Purpose] WHERE [pid]= MachineStockTransferDetails.Purpose) AS Purpose, 
                                            ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, QtyBalance, DeliveredQty, UnitWeight, 
                                            (SELECT  [Company] FROM [Party] WHERE [PartyID]= MachineStockTransferDetails.Customer) AS Customer,
                                            (SELECT [BrandName] FROM [CustomerBrands] WHERE BrandID=MachineStockTransferDetails.BrandID) AS BrandID,
                                            (SELECT [BrandName] FROM [Brands] WHERE BrandID=MachineStockTransferDetails.SizeId) AS SizeId,
                                            (SELECT [DepartmentName] FROM [Colors] WHERE Departmentid=MachineStockTransferDetails.Color) AS Color,
                                            (SELECT [Spec] FROM [Specifications] WHERE id=MachineStockTransferDetails.Spec) AS Spec
                                            FROM MachineStockTransferDetails WHERE MachineID='" + ddMachineFrom.SelectedValue + "' AND EntryBy=@EntryBy AND OrderID='' AND StockType='Raw'  ORDER BY Id", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));

        //cmd.CommandText = "SELECT Id, ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal FROM StockTransferDetails WHERE EntryBy=@EntryBy AND OrderID='' ORDER BY Id";
    ...
    {
        ltrQty.Text = SQLQuery.ReturnString("Select ISNULL(SUM(QtyBalance),0) from MachineStockTransferDetails where OrderID='' AND MachineID='" + ddMachineFrom.SelectedValue + "'") + "pcs, " +
            SQLQuery.ReturnString("Select ISNULL(SUM(UnitWeight),0) from MachineStockTransferDetails where OrderID='' AND MachineID='" + ddMachineFrom.SelectedValue + "'") + "kg";
        //QtyinStock();
    }
    protected void ItemGrid_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            string OrderID = SQLQuery.ReturnString("Select OrderID from MachineStockTransferDetails where Id='" + lblItemCode.Text + "'");
            //string deliveredQty = SQLQuery.ReturnString("Select SUM(DeliveredQty) from StockTransferDetails where OrderID='" + OrderID + "'");

            //if (deliveredQty == "0")
            //{
            SqlCommand cmd7 = new SqlCommand("DELETE MachineStockTransferDetails WHERE Id=" + lblItemCode.Text + "", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (ddMachineFrom.SelectedValue != ddMachineTo.SelectedValue)
            {
                if (btnSave.Text == "Save")
                {
                    if (ltrQty.Text == "0")
                    {
                        ...
                        lblMsg.Text = "ERROR: <br> No items were added to the Transfer!";
                    }
                    else
                    {
                        ExecuteInsert();
                        ...
                        QtyinStock();
                        GridView1.DataBind();
                        Notify("Stock Transfer saved successfully...", "success", lblMsg);
                    }
                }
                else
                {
            ...
            {
                Notify("Please select godown name properly...", "error", lblMsg2);
            }
        }
        catch (Exception ex)
        {
    ...
    private void ExecuteInsert()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO MachineStockin (OrderID, OrderDate, MachineNo, GodownName, LocationID, LocationName, Remarks, TotalQty, EntryBy, ProjectId)" +
                                                    " VALUES (@OrderID, @OrderDate, @MachineNo, @GodownName, @LocationID, @LocationName, @Remarks, @TotalQty, @EntryBy, @ProjectId)", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        cmd2.Parameters.AddWithValue("@MachineNo", ddMachineFrom.SelectedValue);
        cmd2.Parameters.AddWithValue("@GodownName", ddMachineFrom.SelectedItem.Text);
        cmd2.Parameters.AddWithValue("@LocationID", "");

        cmd2.Parameters.AddWithValue("@LocationName", "");
        ...
        SqlCommand cmd = new SqlCommand("UPDATE MachineStockTransferDetails SET OrderID='" + orderNo + "' WHERE  EntryBy=@EntryBy AND OrderID=''  AND StockType='Raw'  AND MachineID='" + ddMachineFrom.SelectedValue + "' ", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        foreach (GridViewRow row in ItemGrid.Rows)
        {
            ...
            string purpose = SQLQuery.ReturnString("SELECT Purpose FROM MachineStockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string sizeId = SQLQuery.ReturnString("SELECT SizeID FROM MachineStockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string brandID = SQLQuery.ReturnString("SELECT brandID FROM MachineStockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string productID = SQLQuery.ReturnString("SELECT productID FROM MachineStockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string ProductName = SQLQuery.ReturnString("SELECT ProductName FROM MachineStockTransferDetails where (Id='" + lblEntryId.Text + "')");

            string itemType = SQLQuery.ReturnString("SELECT ItemType FROM MachineStockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string customer = SQLQuery.ReturnString("SELECT Customer FROM MachineStockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string color = SQLQuery.ReturnString("SELECT Color FROM MachineStockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string spec = SQLQuery.ReturnString("SELECT spec FROM MachineStockTransferDetails where (Id='" + lblEntryId.Text + "')");
            decimal price = Convert.ToDecimal(SQLQuery.ReturnString("SELECT UnitCost FROM MachineStockTransferDetails where (Id='" + lblEntryId.Text + "')"));
            ...
            int qty = Convert.ToInt32(lbliQty.Text); //int outQty = 0;
            decimal weight = Convert.ToDecimal(lblUnitWeight.Text); //decimal outWeight = 0;
            //string type = "Stock-in";
            string detail = "Stock Transfer";
            ...
            Stock.Inventory.SaveToMachineStock(purpose, orderNo, detail, lblEntryId.Text, sizeId, customer, brandID, color, spec, productID, ProductName, itemType, ddMachineFrom.SelectedValue, "", "", ddGroup.SelectedValue, 0, qty, price, 0, weight, "", "Stock-out", "Transfer", "", lName, Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"));
            Stock.Inventory.SaveToMachineStock(purpose, orderNo, detail, lblEntryId.Text, sizeId, customer, brandID, color, spec, productID, ProductName, itemType, ddMachineTo.SelectedValue, "", "", ddGroup.SelectedValue, qty, 0, price, weight, 0, "", "Stock-in", "Transfer", "", lName, Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"));
            ...
            //string companyFor = SQLQuery.ReturnString("SELECT Company from Party where PartyID=(Select CompanyFor FROM StockTransferDetails where (Id='" + lblEntryId.Text + "'))");
    ...
    {
        string lName = Page.User.Identity.Name.ToString();
        SqlCommand cmd = new SqlCommand("Delete MachineStockTransferDetails WHERE EntryBy=@EntryBy AND OrderID=''  AND MachineID='" + ddMachineFrom.SelectedValue + "' ", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Parameters.AddWithValue("@EntryBy", lName);
        cmd.Connection.Open();
        cmd.ExecuteNonQuery();
        cmd.Connection.Close();
    ...
    private void QtyinStock()
    {
        try
        {
            ...
            if (ddSubGroup.SelectedValue == "10") //printing ink
            {
                ...
                txtCurrentKg.Text = Stock.Inventory.AvailableInkWeight(ddProduct.SelectedValue, ddSpec.SelectedValue, ddMachineFrom.SelectedValue, "");
            ...
            {
                txtCurrentQty.Text = "0";
                decimal inputAvailable = Convert.ToDecimal(Stock.Inventory.QtyinMachineStock(ddPurpose.SelectedValue, ddProduct.SelectedValue, ddMachineFrom.SelectedValue));

                txtCurrentKg.Text = "0";// inputAvailable.ToString();
                txtPrice.Visible = true;
                txtPrice.Text = Stock.Inventory.LastPlasticRawPriceMachine(ddPurpose.SelectedValue, ddProduct.SelectedValue);
            ...
            {
                txtCurrentQty.Text = "0";
                txtCurrentKg.Text = Stock.Inventory.PlasticRawWeightMachine(ddPurpose.SelectedValue, ddProduct.SelectedValue, ddMachineFrom.SelectedValue);
                txtPrice.Visible = true;
                txtPrice.Text = Stock.Inventory.LastPlasticRawPriceMachine(ddProduct.SelectedValue, ddSpec.SelectedValue);
                txtQty.Text = "0";
                txtWeight.Focus();
            }
            else
            {
                if (ddType.SelectedValue == "Printed Sheet")
                {
                    txtCurrentQty.Text = Stock.Inventory.AvailablePrintedQty(ddPurpose.SelectedValue, ddType.SelectedValue, ddProduct.SelectedValue,
                        ddCustomer.SelectedValue, ddBrand.SelectedValue, ddSize.SelectedValue, ddColor.SelectedValue, ddMachineFrom.SelectedValue, "");
                ...
                else
                {
                    txtCurrentQty.Text = Stock.Inventory.AvailableNonPrintedQty(ddPurpose.SelectedValue, ddType.SelectedValue, ddProduct.SelectedValue,
                        ddMachineFrom.SelectedValue, "");

                    ltrCQty.Text = txtCurrentQty.Text;
                    txtCurrentKg.Text = Stock.Inventory.AvailableNonPrintedWeight(ddPurpose.SelectedValue, ddType.SelectedValue, ddProduct.SelectedValue, ddMachineFrom.SelectedValue, "");
                    txtPrice.Visible = true;
    ...
    {
        SqlCommand cmd = new SqlCommand(@"SELECT ItemType, ProductID, SizeId, Customer, BrandID, Color, Quantity, UnitWeight, MachineID FROM [MachineStockTransferDetails] WHERE Id='" + entryID + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Connection.Open();
        ...
        if (dr.Read())
        {
            ...
            ddMachineFrom.SelectedValue = dr[8].ToString();
