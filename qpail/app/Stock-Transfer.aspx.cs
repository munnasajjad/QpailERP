...
public partial class app_Stock_Transfer : System.Web.UI.Page
{
        ...
        try
        {
            if (Convert.ToDecimal(txtCurrentQty.Text) >= Convert.ToDecimal(txtQty.Text) && Convert.ToDecimal(txtCurrentKg.Text) >= Convert.ToDecimal(txtWeight.Text))
            {
                if (btnAdd.Text == "Add")
                {
                    ...
                    SqlCommand cmde = new SqlCommand("SELECT ProductName FROM StockTransferDetails WHERE  ProductID ='" + ddProduct.SelectedValue + "' AND  OrderID ='' AND EntryBy ='" + lName + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
                ...
                {
                    //ExecuteUpdate();
                    SQLQuery.ExecNonQry("Delete StockTransferDetails where (id ='" + lblOrderID.Text + "')");
                    InsertData();
                    btnAdd.Text = "Add";
    ...
    private void InsertData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO StockTransferDetails (StockType, purpose, ItemType, OrderID, ProductID, ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor, GodownID, LocationID, QtyBalance, DeliveredQty, Spec) " +
                                         "VALUES ('Raw', '" + ddPurpose.SelectedValue + "', '" + ddType.SelectedValue + "', @OrderID, @ProductID, @ProductName, @UnitCost, @Quantity, @UnitWeight, '" + ltrUnit.Text + "', @ItemTotal, @TotalWeight, @EntryBy, @CompanyFor, '" + ddGodownFrom.SelectedValue + "', '" + ddLocationFrom.SelectedValue + "','" + balance + "', '" + ltrCQty.Text + "', '" + spec + "'  )", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));

        if (ddType.SelectedValue == "Printed Sheet")
        {
            cmd2 = new SqlCommand(@"INSERT INTO StockTransferDetails (StockType, purpose, ItemType, OrderID, SizeId, ProductID, Customer, BrandID, ProductName, Color, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor, GodownID, LocationID, QtyBalance, DeliveredQty)" +
                                        " VALUES ('Raw', '" + ddPurpose.SelectedValue + "', '" + ddType.SelectedValue + "', @OrderID, @SizeId, @ProductID, '" + ddCustomer.SelectedValue + "', @BrandID, @ProductName, '" + ddColor.SelectedValue + "', @UnitCost, @Quantity, @UnitWeight, '" + ltrUnit.Text + "', @ItemTotal, @TotalWeight, @EntryBy, @CompanyFor, '" + ddGodownFrom.SelectedValue + "', '" + ddLocationFrom.SelectedValue + "','" + balance + "', '" + ltrCQty.Text + "'  )", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        }
    ...
    {
        string lName = Page.User.Identity.Name.ToString();
        SqlCommand cmd = new SqlCommand(@"SELECT  Id, (SELECT  [Purpose] FROM [Purpose] WHERE [pid]= StockTransferDetails.Purpose) AS Purpose, 
                                            ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, QtyBalance, DeliveredQty, UnitWeight, 
                                            (SELECT  [Company] FROM [Party] WHERE [PartyID]= StockTransferDetails.Customer) AS Customer,
                                            (SELECT [BrandName] FROM [CustomerBrands] WHERE BrandID=StockTransferDetails.BrandID) AS BrandID,
                                            (SELECT [BrandName] FROM [Brands] WHERE BrandID=StockTransferDetails.SizeId) AS SizeId,
                                            (SELECT [DepartmentName] FROM [Colors] WHERE Departmentid=StockTransferDetails.Color) AS Color,
                                            (SELECT [Spec] FROM [Specifications] WHERE id=StockTransferDetails.Spec) AS Spec
                                            FROM StockTransferDetails WHERE GodownID='" + ddGodownFrom.SelectedValue + "' AND LocationID = '"+ddLocationFrom.SelectedValue+"' AND EntryBy=@EntryBy AND OrderID='' AND StockType='Raw'  ORDER BY Id", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    {
        ltrQty.Text = SQLQuery.ReturnString("SELECT ISNULL(SUM(QtyBalance),0) FROM StockTransferDetails WHERE OrderID='' AND GodownID='" + ddGodownFrom.SelectedValue + "' AND LocationID = '" + ddLocationFrom.SelectedValue + "'") + "pcs, " +
            SQLQuery.ReturnString("SELECT ISNULL(SUM(UnitWeight),0) FROM StockTransferDetails WHERE OrderID='' AND GodownID='" + ddGodownFrom.SelectedValue + "' AND LocationID = '" + ddLocationFrom.SelectedValue + "'") + "kg";
        //QtyinStock();
    }
    protected void ItemGrid_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            string OrderID = SQLQuery.ReturnString("Select OrderID from StockTransferDetails where Id='" + lblItemCode.Text + "'");
            //string deliveredQty = SQLQuery.ReturnString("Select SUM(DeliveredQty) from StockTransferDetails where OrderID='" + OrderID + "'");

            //if (deliveredQty == "0")
            //{
            SqlCommand cmd7 = new SqlCommand("DELETE StockTransferDetails WHERE Id=" + lblItemCode.Text + "", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (ddGodownFrom.SelectedValue != ddGodownTo.SelectedValue)
            {
    ...
    private void ExecuteInsert()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO Stockin (OrderID, OrderDate, GodownID, GodownName, LocationID, LocationName, ToGodownID, ToLocationID, Remarks, TotalQty, EntryBy, ProjectId)" +
        ...
        cmd2.Parameters.AddWithValue("@GodownID", ddGodownFrom.SelectedValue);
        cmd2.Parameters.AddWithValue("@GodownName", ddGodownFrom.SelectedItem.Text);
        cmd2.Parameters.AddWithValue("@LocationID", ddLocationFrom.SelectedValue);
        cmd2.Parameters.AddWithValue("@LocationName", ddLocationFrom.SelectedItem.Text);
        cmd2.Parameters.AddWithValue("@ToGodownID", ddGodownTo.SelectedValue);
        cmd2.Parameters.AddWithValue("@ToLocationID", ddLocationTo.SelectedValue);
        ...
        SqlCommand cmd = new SqlCommand("UPDATE StockTransferDetails SET OrderID='" + orderNo + "' WHERE  EntryBy=@EntryBy AND OrderID=''  AND StockType='Raw'  AND GodownID='" + ddGodownFrom.SelectedValue + "' AND LocationID = '" + ddLocationFrom.SelectedValue + "' ", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        foreach (GridViewRow row in ItemGrid.Rows)
        {
            ...
            string purpose = SQLQuery.ReturnString("SELECT Purpose FROM StockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string sizeId = SQLQuery.ReturnString("SELECT SizeID FROM StockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string brandID = SQLQuery.ReturnString("SELECT brandID FROM StockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string productID = SQLQuery.ReturnString("SELECT productID FROM StockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string ProductName = SQLQuery.ReturnString("SELECT ProductName FROM StockTransferDetails where (Id='" + lblEntryId.Text + "')");

            string itemType = SQLQuery.ReturnString("SELECT ItemType FROM StockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string customer = SQLQuery.ReturnString("SELECT Customer FROM StockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string color = SQLQuery.ReturnString("SELECT Color FROM StockTransferDetails where (Id='" + lblEntryId.Text + "')");
            string spec = SQLQuery.ReturnString("SELECT spec FROM StockTransferDetails where (Id='" + lblEntryId.Text + "')");
            decimal price = Convert.ToDecimal(SQLQuery.ReturnString("SELECT UnitCost FROM StockTransferDetails where (Id='" + lblEntryId.Text + "')"));
            ...
            decimal qty = Convert.ToDecimal(lbliQty.Text); //int outQty = 0;
            ...
            Stock.Inventory.SaveToStock(purpose, orderNo, detail, lblEntryId.Text, sizeId, customer, brandID, color, spec, productID, ProductName, itemType, ddGodownFrom.SelectedValue, ddLocationFrom.SelectedValue, ddGroup.SelectedValue, 0, qty, price, 0, weight, "", "Stock-out", "Transfer", "", lName, Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"));
            Stock.Inventory.SaveToStock(purpose, orderNo, detail, lblEntryId.Text, sizeId, customer, brandID, color, spec, productID, ProductName, itemType, ddGodownTo.SelectedValue, ddLocationTo.SelectedValue, ddGroup.SelectedValue, qty, 0, price, weight, 0, "", "Stock-in", "Transfer", "", lName, Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"));
    ...
    {
        string lName = Page.User.Identity.Name.ToString();
        SqlCommand cmd = new SqlCommand("Delete StockTransferDetails WHERE EntryBy=@EntryBy AND OrderID=''  AND WarehouseID='" + ddGodownFrom.SelectedValue + "' ", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Parameters.AddWithValue("@EntryBy", lName);
        cmd.Connection.Open();
        cmd.ExecuteNonQuery();
        cmd.Connection.Close();
    ...
    private void QtyinStock()
    {
        try
        {
            ...
            if (ddSubGroup.SelectedValue == "10") //printing ink
            {
                ...
                txtCurrentKg.Text = Stock.Inventory.AvailableInkWeight(ddProduct.SelectedValue, ddSpec.SelectedValue, ddGodownFrom.SelectedValue, ddLocationFrom.SelectedValue);
            ...
            {
                txtCurrentQty.Text = "0";
                decimal inputAvailable = Convert.ToDecimal(Stock.Inventory.QtyinStock(ddPurpose.SelectedValue, ddProduct.SelectedValue, ddGodownFrom.SelectedValue, ddLocationFrom.SelectedValue));
            ...
            {
                txtCurrentQty.Text = "0";
                txtCurrentKg.Text = Stock.Inventory.PlasticRawWeight(ddPurpose.SelectedValue, ddProduct.SelectedValue, ddGodownFrom.SelectedValue, ddLocationFrom.SelectedValue);
                txtPrice.Visible = true;
            ...
            else
            {
                if (ddType.SelectedValue == "Printed Sheet")
                {
                    txtCurrentQty.Text = Stock.Inventory.AvailablePrintedQty(ddPurpose.SelectedValue, ddType.SelectedValue, ddProduct.SelectedValue,
                        ddCustomer.SelectedValue, ddBrand.SelectedValue, ddSize.SelectedValue, ddColor.SelectedValue, ddGodownFrom.SelectedValue, ddLocationFrom.SelectedValue);
                ...
                else
                {
                    txtCurrentQty.Text = Stock.Inventory.AvailableNonPrintedQty(ddPurpose.SelectedValue, ddType.SelectedValue, ddProduct.SelectedValue,
                        ddGodownFrom.SelectedValue, ddLocationFrom.SelectedValue);

                    ltrCQty.Text = txtCurrentQty.Text;
                    txtCurrentKg.Text = Stock.Inventory.AvailableNonPrintedWeight(ddPurpose.SelectedValue, ddType.SelectedValue, ddProduct.SelectedValue, ddGodownFrom.SelectedValue, ddLocationFrom.SelectedValue);
                    txtPrice.Visible = true;
    ...
    {
        SqlCommand cmd = new SqlCommand(@"SELECT ItemType, ProductID, SizeId, Customer, BrandID, Color, Quantity, UnitWeight, GodownID, LocationID FROM [StockTransferDetails] WHERE Id='" + entryID + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Connection.Open();
        ...
        if (dr.Read())
        {
            ...
            ddGodownFrom.SelectedValue = dr[8].ToString();
            ddLocationFrom.SelectedValue = dr[9].ToString();
