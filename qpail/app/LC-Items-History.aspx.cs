...
public partial class app_LC_Items_History : System.Web.UI.Page
{
        ...
        try
        {
            ...
            if (txtInvDate.Text != "")
            {
                try
                {
                    ...
                    invDate = " AND a.ReceiveDate  >= '" + invDate + "'";
                }
                catch (Exception) { }
            }
            ...
            if (txtPODate.Text != "")
            {
                try
                {
                    ...
                    poDate = " AND a.ReceiveDate <= '" + poDate + "'";
                }
                catch (Exception) { }
            }

            string query = customer + invNo + invDate + poDate;
            DataSet ds = SQLQuery.ReturnDataSet(@"SELECT   (Select AccountsHeadName from HeadSetup WHERE AccountsHeadID=a.LCNo) AS LCNo,
                             (SELECT        Purpose
                               FROM            Purpose
                               WHERE        (pid = a.Purpose)) AS Purpose, a.ReceiveDate,
                             (SELECT        spec
                               FROM            Specifications
                               WHERE        (id = a.Spec)) AS spec, qty, UnitPrice, TotalBDT, UnitCostBDT, UsedQty
                        FROM LcItems AS a WHERE        (LCType = 'Voucher') " + query + " ORDER BY a.EntryID desc");
