...
public partial class app_Stock_Adj_Processed : System.Web.UI.Page
{
    ...
    private void Notify(string msg, string type)
    {
        ...
        lblMsg2.Attributes.Add("class", "xerp_" + type);
        lblMsg2.Text = msg;
    }
    ...
    private void InsertData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand(@"INSERT INTO StockinDetailsRaw (StockType, purpose, ItemType, OrderID, SizeId, ProductID, Customer, BrandID, ProductName, Color, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor, GodownID, LocationID, QtyBalance, DeliveredQty)" +
                                        " VALUES ('Processed','" + ddPurpose.SelectedValue + "','', @OrderID, @SizeId, @ProductID, '" + ddCustomer.SelectedValue + "', @BrandID, @ProductName, '" + ddColor.SelectedValue + "', @UnitCost, @Quantity, @UnitWeight, '" + ltrUnit.Text + "', @ItemTotal, @TotalWeight, @EntryBy, @CompanyFor, '" + ddGodown.SelectedValue + "', '"+ddLocation.SelectedValue+"','" + balance + "', '" + ltrCQty.Text + "'  )", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
 ...
 (SELECT  [Purpose] FROM [Purpose] WHERE [pid]= StockinDetailsRaw.Purpose) AS Purpose, 
...
FROM StockinDetailsRaw WHERE GodownID='" + ddGodown.SelectedValue + "' AND LocationID = '"+ddLocation.SelectedValue+"' AND EntryBy=@EntryBy AND OrderID='' AND StockType='Processed' ORDER BY Id", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    private void ExecuteInsert()
    {
        ...
        SqlCommand cmd = new SqlCommand("UPDATE StockinDetailsRaw SET OrderID='" + orderNo + "' WHERE  EntryBy=@EntryBy AND OrderID=''  AND StockType='Processed'  AND GodownID='" + ddGodown.SelectedValue + "' AND LocationID = '" + ddLocation.SelectedValue + "' ", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        foreach (GridViewRow row in ItemGrid.Rows)
        {
            ...
            string purpose = RunQuery.SQLQuery.ReturnString("SELECT Purpose FROM StockinDetailsRaw where (Id='" + lblEntryId.Text + "')");
    ...
    private void QtyinStock()
    {
        try
        {
            ...
            txtCurrentQty.Text = Stock.Inventory.AvailableProcessedQty(ddPurpose.SelectedValue, "", ddProduct.SelectedValue,
                ...
                txtCurrentKg.Text = Stock.Inventory.AvailableProcessedWeight(ddPurpose.SelectedValue, "", ddProduct.SelectedValue,
                    ddCustomer.SelectedValue, ddBrand.SelectedValue, ddSize.SelectedValue, ddColor.SelectedValue,
                    ddGodown.SelectedValue, ddLocation.SelectedValue);
