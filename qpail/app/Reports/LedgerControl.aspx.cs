...
public partial class Application_Reports_LedgerControl : System.Web.UI.Page
{
    ...
    private void LoadGridData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("SELECT [EntryDate] as TrDate, AccountsHeadName, [VoucherRowDescription] as Description, [VoucherDR] As Dr, [VoucherCR] As Cr, [VoucherNo] As Balance  FROM [VoucherDetails] WHERE ISApproved='A' and  EntryDate >= @DateFrom AND EntryDate <= @DateTo and ([AccountsHeadID] in (Select AccountsHeadID from HeadSetup where ControlAccountsID= @HeadName)) ORDER BY [SerialNo] ASC", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd2.Parameters.Add("@HeadName", SqlDbType.VarChar).Value = ddParties.SelectedValue;
        cmd2.Parameters.Add("@DateFrom", SqlDbType.DateTime).Value = Convert.ToDateTime(txtDateFrom.Text).AddDays(-1).ToShortDateString();
        cmd2.Parameters.Add("@DateTo", SqlDbType.DateTime).Value = Convert.ToDateTime(txtDateTo.Text);
        ...
        dt1.Columns.Add(new DataColumn("AcHead", typeof(string)));
        ...
        SqlCommand cmd2x = new SqlCommand("SELECT isnull(sum(OpBalDr),0) - isnull(sum(OpBalCr),0) FROM [HeadSetup] WHERE ([ControlAccountsID] = @HeadName)", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        cmd2y.Parameters.Add("@DateFrom", SqlDbType.DateTime).Value = Convert.ToDateTime(txtDateFrom.Text).AddDays(-2).ToShortDateString();
        ...
        if (isLiability == "02")
        {
             opBal = Convert.ToDecimal(RunQuery.SQLQuery.ReturnString("SELECT  isnull(sum(OpBalCr),0) - isnull(sum(OpBalDr),0)  FROM [HeadSetup] WHERE ([ControlAccountsID] = '" + ddParties.SelectedValue + "')"));
             preBal = Convert.ToDecimal(RunQuery.SQLQuery.ReturnString("SELECT  isnull(sum(VoucherCR),0) - isnull(sum(VoucherDR),0) FROM [VoucherDetails] WHERE ([AccountsHeadID]  in (Select AccountsHeadID from HeadSetup where ControlAccountsID= '" + ddParties.SelectedValue + "')) and   EntryDate < '" + Convert.ToDateTime(txtDateFrom.Text).ToString("yyyy-MM-dd") + "'  and ISApproved='A'"));
        }
        ...
        if (recordcount > 0)
        {
            do
            {
                ...
                string acHead = ds.Tables[0].Rows[ic]["AccountsHeadName"].ToString();
        ...
        if (isLiability == "02")
        {
            cmd2z = new SqlCommand("SELECT isnull(sum(VoucherCR),0)-isnull(sum(VoucherDR),0) as balance FROM [VoucherDetails] WHERE ([AccountsHeadID] in (Select AccountsHeadID from HeadSetup where ControlAccountsID= @HeadName)) and EntryDate <= @DateTo and ISApproved='A'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        }

        cmd2z.Parameters.Add("@HeadName", SqlDbType.VarChar).Value = ddParties.SelectedValue;
        cmd2z.Parameters.Add("@DateTo", SqlDbType.DateTime).Value = Convert.ToDateTime(txtDateTo.Text).AddSeconds(86399);
        ...
        dr1["Dr"] = 0;
        dr1["Cr"] = 0;
        dr1["Balance"] = currBal + (opBal + preBal);
        dt1.Rows.Add(dr1);
