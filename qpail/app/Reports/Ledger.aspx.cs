...
public partial class Application_Reports_Ledger : System.Web.UI.Page
{
    ...
    private void LoadGridData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("SELECT [TrDate], [Description], [Dr], [Cr], [Balance] FROM [Transactions] WHERE ([HeadName] = @HeadName) and   TrDate >= @DateFrom AND TrDate <= @DateTo ORDER BY [TrID] ASC", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        SqlCommand cmd2x = new SqlCommand("SELECT isnull(sum(Dr),0) FROM [Transactions] WHERE ([HeadName] = @HeadName) and   TrDate <= @DateFrom", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));        
        ...
        SqlCommand cmd2y = new SqlCommand("SELECT isnull(sum(Cr),0) FROM [Transactions] WHERE ([HeadName] = @HeadName) and   TrDate <= @DateFrom", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        //get closing balance        
        SqlCommand cmd2z = new SqlCommand("SELECT isnull(sum(Dr),0)-isnull(sum(Cr),0) as balance FROM [Transactions] WHERE ([HeadName] = @HeadName) and TrDate <= @DateTo", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
