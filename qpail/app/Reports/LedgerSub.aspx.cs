...
public partial class Application_Reports_LedgerSub : System.Web.UI.Page
{
    ...
    private void LoadGridData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("SELECT [EntryDate], [AccountsHeadName], [VoucherDR], [VoucherCR], [VoucherNo], VoucherRowDescription FROM [VoucherDetails] WHERE ISApproved='A' and  EntryDate >= @DateFrom AND EntryDate <= @DateTo and ([AccountsHeadID] in (Select AccountsHeadID from HeadSetup where AccountsID= @HeadName)) ORDER BY [SerialNo] ASC", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        dt1.Columns.Add(new DataColumn("VoucherRowDescription", typeof(string)));
        ...
        string date; string AccountsHeadName; string desc;

        //get openning balance        
        SqlCommand cmd2x = new SqlCommand("SELECT isnull(sum(VoucherDR),0) FROM [VoucherDetails] WHERE ([AccountsHeadID] in (Select AccountsHeadID from HeadSetup where AccountsID= @HeadName)) and   EntryDate <= @DateFrom and ISApproved='A'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        SqlCommand cmd2y = new SqlCommand("SELECT isnull(sum(VoucherCR),0) FROM [VoucherDetails] WHERE ([AccountsHeadID] in (Select AccountsHeadID from HeadSetup where AccountsID= @HeadName)) and   EntryDate <= @DateFrom  and ISApproved='A'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        dr1["AccountsHeadName"] = "Openning Balance";
        dr1["VoucherDR"] = 0;
        dr1["VoucherCR"] = 0;
        dr1["Balance"] = currBal;
        dt1.Rows.Add(dr1);
        ...
        if (recordcount > 0)
        {
            do
            {
                ...
                desc = ds.Tables[0].Rows[ic]["VoucherRowDescription"].ToString();
                ...
                dr1["VoucherRowDescription"] = desc;
                dr1["VoucherDR"] = debt;
        ...
        dr1["AccountsHeadName"] = "";
        dr1["VoucherRowDescription"] = "Closing Balance";
        dr1["VoucherDR"] = 0;
        dr1["VoucherCR"] = 0;
        dr1["Balance"] = currBal;
        dt1.Rows.Add(dr1);
