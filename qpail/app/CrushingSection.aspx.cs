...
public partial class app_CrushingSection : System.Web.UI.Page
{
...
FROM StockinDetailsRaw WHERE GodownID='" + ddGodown.SelectedValue + "' AND  EntryBy=@EntryBy AND OrderID='' AND StockType='Waste'  ORDER BY Id", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));

        //AND LocationID = '" + ddLocation.SelectedValue + "' AND   cmd.CommandText = "SELECT Id, ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal FROM StockinDetailsRaw WHERE EntryBy=@EntryBy AND OrderID='' ORDER BY Id";
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (btnSave.Text == "Save")
            {
                ...
                lblMsg.Text = "Crushing Input Saved Successfully...";
                //}
            }
            else
            {
    ...
    private void ExecuteInsert()
    {
        ...
        string lName = Page.User.Identity.Name.ToString();
        decimal crushingOutputWeight = 0;
        ...
        foreach (GridViewRow row in ItemGrid.Rows)
        {
            ...
            string type = "Stock-in"; string detail = "Crushed Items";
        ...
        SQLQuery.Empty2Zero(txtCrushingWeight);
        if (Convert.ToDecimal(txtCrushingWeight.Text) > 0)
        {
            crushingOutputWeight = Convert.ToDecimal(txtCrushingWeight.Text);
        }

        string productName =
            SQLQuery.ReturnString("SELECT ItemName FROM Products WHERE ProductID = '" + ddItemSelection.SelectedValue + "'");
        Stock.Inventory.SaveToStock("", orderNo, "Crushing Items", "", "", "", "", "", "", ddItemSelection.SelectedValue, productName, "Crushed Items", ddGodown.SelectedValue, ddLocation.SelectedValue, ddGroup.SelectedValue, crushingOutputWeight, 0, 0, crushingOutputWeight, 0, "", "Stock-in", "Crushing", ddLocation.SelectedItem.Text, lName, Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"));
    }
