...
public partial class app_Order_Close : System.Web.UI.Page
{
    ...
    private void LoadPO()
    {
        if (rbClose.Checked)
        {
            RunQuery.SQLQuery.PopulateDropDown("SELECT [OrderID] FROM [Orders] WHERE CustomerName='" + ddCustomer.SelectedValue + "' AND ([DeliveryStatus] = 'A' OR [DeliveryStatus] = 'P') ORDER BY OrderSl DESC", ddOrders, "OrderID", "OrderID");    
        }
        else
        {
            RunQuery.SQLQuery.PopulateDropDown("SELECT [OrderID] FROM [Orders] WHERE CustomerName='" + ddCustomer.SelectedValue + "' AND ([DeliveryStatus] = 'D' OR [DeliveryStatus] = 'C') ORDER BY OrderSl DESC", ddOrders, "OrderID", "OrderID");    
        }
    }
    ...
    private void BindItemGrid()
    {
        if (ddOrders.SelectedValue != "")
        {
            ...
            SqlCommand cmd = new SqlCommand("SELECT Id, ProductName, UnitCost, Quantity, DeliveredQty, UnitWeight, UnitType, ItemTotal, QtyBalance FROM OrderDetails WHERE OrderID='" + OrderID + "' ORDER BY Id", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    private void UpdateOrder()
    {
        ...
        string deliveryStatus = "P";
        if (rbClose.Checked)
        {
            deliveryStatus = "D";
        }

        RunQuery.SQLQuery.ExecNonQry("Update Orders set DeliveryStatus='" + deliveryStatus + "', UpdateBy='" + lName + "', UpdateDate='"+DateTime.Now.ToString("yyyy-MM-dd")+"' WHERE OrderID='" + ddOrders.SelectedValue + "'");
    }
