...
public partial class app_rpt_Prdn_Plastic_Finish : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        ...
        string url = SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") +
            "XerpReports/FifoSummaryReport.aspx?customer=" + ddCustomer.SelectedValue + "&brand=" + ddBrand.SelectedValue + "&purpose=" + ddPurpose.SelectedValue + "&itemName=" + ddItemName.SelectedValue + "&pCcolor=" + ddColor.SelectedValue + "&dateFrom=" + dateFrom + "&dateTo=" + dateTo;
    ...
    private DataTable BindItemGrid()
    {
        ...
        try
        {
            ...
            //{
            //    Customer = " AND Company='" + ddCustomer.SelectedValue + "' ";
            //}
            //string Brand = " ";
            //if (ddBrand.SelectedValue != "--- all ---")
            //{
            //    Brand = " AND BrandName='" + ddBrand.SelectedValue + "' ";
            //}
            string item = " ";
            if (ddItemName.SelectedValue != "--- all ---")
            {
                item = " AND (ItemName = '" + ddItemName.SelectedValue + "')  ";
            }
            ...
            string query = " FROM PrdnPlasticFinish a where pid<>0 "+ item + dateFrom + dateTo;
            ...
            query = @" SELECT (CONVERT(varchar,Date,103) +'') AS Date, (Select Company from Party where PartyID=a.CustomerID) AS  [Customer], 
                        ...
                        (SELECT ItemName FROM Products WHERE (ProductID = a.ItemName)) AS [Product Name], 
                          (SELECT [DepartmentName] FROM [Colors] Where Departmentid=(Select TOP(1) Color from PrdnPlasticFinishDetails where PrdnID=a.ProductionID)) as Color, FinalOutput as Qty, OutputWeight as Weight " + query;
