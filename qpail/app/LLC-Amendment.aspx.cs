...
public partial class app_LLC_Amendment : System.Web.UI.Page
{
    ...
    private void BindItemGrid()
    {
        ...
        if (OrderID != "")
        {
            ...
            SqlCommand cmd2 = new SqlCommand("SELECT PiDate, LcNo, LcDate, LcExpDate, IssuingBank, IssueingBankBranch, AdvisingBankAccount, LcTerms, AmmendmentNo, AmmendmentDate FROM [PI] WHERE LcNo='" + OrderID + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
            ...
            if (dr2.Read())
            {
                ...
                ddIssueBank.SelectedValue = dr2[4].ToString();
                txtBranch.Text = dr2[5].ToString();
                ddBank.SelectedValue = dr2[6].ToString();
                txtTerms.Text = dr2[7].ToString();
                txtAmdNo.Text = dr2[8].ToString();
                txtAmdDate.Text = dr2[9].ToString();
            }
    ...
    protected void ItemGrid_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        string wt = RunQuery.SQLQuery.ReturnString("Select ISNULL(SUM(TotalWeight),0) from OrderDetails where OrderID='" + ddOrders.SelectedValue + "'");
        ltrWeight.Text=Convert.ToString(Convert.ToDecimal(wt)/1000M);
    }
    ...
    protected void btnSave_Click(object sender, EventArgs e)
    {
        try
        {
            if (txtAmdNo.Text != "" && txtAmdDate.Text!="")
            {
                ...
                if (txtEditLLC.Text != "")
                {
                    RunQuery.SQLQuery.ExecNonQry("UPDATE PI SET LcNo='" + txtEditLLC.Text + "' WHERE LcNo='" + ddOrders.SelectedValue + "'");
                    RunQuery.SQLQuery.ExecNonQry("UPDATE OrderDetails SET OrderID='" + txtEditLLC.Text + "'  WHERE OrderID='" + ddOrders.SelectedValue + "'");
                    RunQuery.SQLQuery.ExecNonQry("UPDATE Orders SET OrderID='" + txtEditLLC.Text + "' WHERE OrderID='" + ddOrders.SelectedValue + "'");
                    RunQuery.SQLQuery.ExecNonQry("UPDATE PO_Images SET PONo='" + txtEditLLC.Text + "' WHERE PONo='" + ddOrders.SelectedValue + "'");
                }
                ...
                lblMsg.Text = "LC info updated successfully.";
            }
            else
            {
                txtAmdNo.Focus();
                lblMsg.Attributes.Add("class", "xerp_error");
                lblMsg.Text = "Please input ammendment no. & date";
            }
        }
        catch (Exception ex)
        {
    ...
    private void InsertItemAmmendmentInfo()
    {
        string lName = Page.User.Identity.Name.ToString();
        string productName = ddSize.SelectedItem.Text + " " + ddBrand.SelectedItem.Text + " " + ddProduct.SelectedItem.Text;
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO AmmendmentDetails (OrderID, SizeId, ProductID, BrandID, ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy) VALUES (@OrderID, @SizeId, @ProductID, @BrandID, @ProductName, @UnitCost, @Quantity, @UnitWeight, '" + ltrUnit.Text + "', @ItemTotal, @TotalWeight, @EntryBy)", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        cmd2 = new SqlCommand("UPDATE AmmendmentDetails SET SizeId='" + ddSize.SelectedValue + "', ProductID='" + ddProduct.SelectedValue + "', BrandID='" + ddBrand.SelectedValue + "'," +
    ...
    private void UpdateOrder()
    {
        ...
        RunQuery.SQLQuery.ExecNonQry("UPDATE Orders SET TotalAmount='" + txtTotal.Text + "', Vat='" + txtVat.Text + "', PayableAmount='" + txtPay.Text + "', DeliveryAddress='" + txtAddress.Text + "', DeliveryDate='" + Convert.ToDateTime(txtDeliveryDate.Text).ToString("yyyy-MM-dd") + "'   WHERE OrderID='" + ddOrders.SelectedValue + "'");
        RunQuery.SQLQuery.ExecNonQry("Update PI set LcDate='" + lcDate + "', LcExpDate='" + expDate + "', IssuingBank='" + ddIssueBank.SelectedValue + "', IssueingBankBranch='" + txtBranch.Text + "', AdvisingBankAccount='" + ddBank.SelectedValue + "', LcTerms='" + txtTerms.Text + "', AmmendmentNo='" + txtAmdNo.Text + "', AmmendmentDate='" + txtAmdDate.Text + "' Where LcNo='" + ddOrders.SelectedValue + "'");
        RunQuery.SQLQuery.ExecNonQry("UPDATE PO_Images SET PONo='" + ddOrders.SelectedValue + "', ImgDescription='LC Amendment' WHERE EntryBy='" + lName + "' AND (PONo='' OR PONo='" + ddOrders.SelectedValue + "')");
    ...
    private void InsertHistory()
    {
        ...
        string OrderID=ddOrders.SelectedValue;
            ...
            if (dr.Read())
            {
                string orderdate= Convert.ToDateTime(dr[0].ToString()).ToShortDateString();
                string deliveryDate = Convert.ToDateTime(dr[1].ToString()).ToShortDateString();
                string customer = dr[2].ToString();
                string delAddress = dr[3].ToString();
                string total = dr[4].ToString();
                //txtDiscount.Text = dr[5].ToString();
                string vat = dr[6].ToString();
                string payable = dr[7].ToString();
            
            cmd7.Connection.Close();

            SqlCommand cmd2 = new SqlCommand("SELECT PiDate, LcNo, LcDate, LcExpDate, IssuingBank, IssueingBankBranch, AdvisingBankAccount, LcTerms FROM [PI] WHERE LcNo='" + OrderID + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
            cmd2.Connection.Open();
            SqlDataReader dr2 = cmd2.ExecuteReader();
            if (dr2.Read())
            {
                string piDate = Convert.ToDateTime(dr2[0].ToString()).ToShortDateString();
                string lcNo = dr2[1].ToString();
                string lcdate = Convert.ToDateTime(dr2[2].ToString()).ToShortDateString();
                string expdate = Convert.ToDateTime(dr2[3].ToString()).ToShortDateString();

                string issueBank = dr2[4].ToString();
                string issueBankBranch = dr2[5].ToString();
                string advBankAcc = dr2[6].ToString();
                string lcTerms = dr2[7].ToString();
                ...
                cmd.CommandText = "INSERT INTO PIhistory (LLCNo, LCType, orderdate, deliveryDate, customer, delAddress, total, vat, payable, piDate, lcNo, lcdate, ExpDate, AgentID, issueBank, issueBankBranch, advBankAcc, lcTerms, AmdNo, AmdDate, EntryBy)" +
                                        "VALUES ('" + ddOrders.SelectedValue + "', 'LLC', '" + orderdate + "', '" + deliveryDate + "', '" + customer + "', '" + delAddress + "', '" + total + "', '" + vat + "', '" + payable + "', '" + piDate + "', '" + lcNo + "', '" + lcdate + "', '" + expdate + "', ' ', '" + issueBank + "', '" + issueBankBranch + "', '" + advBankAcc + "', '" + lcTerms + "', '" + txtAmdNo.Text + "', '" + txtAmdDate.Text + "', '" + Page.User.Identity.Name.ToString() + "')";
