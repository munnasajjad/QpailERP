...
public partial class app_Sale_Grade_Report : System.Web.UI.Page
{
    ...
    private DataTable LoadGridData()
    {
        ...
        dt1.Columns.Add(new DataColumn("ItemGrade", typeof(string)));
        ...
        //string query = " AND InvNo IN (Select InvNo from sales where CustomerID = '" + ddCustomer.SelectedValue + "' and DeliveryDate >= '" + dtFrom.ToString("yyyy-MM-dd") + "' AND DeliveryDate <= '" + dtTo.ToString("yyyy-MM-dd") + "')  ";
        //DataTable dt = RunQuery.SQLQuery.ReturnDataTable("SELECT [InvNo], [ProductName], [Quantity], [UnitType], [UnitCost], [ItemTotal], [UnitWeight], [TotalWeight], [VAT], NetAmount, [EntryDate], [ReturnQty] FROM [SaleDetails] WHERE  EntryDate >= '" + dtFrom.ToString("yyyy-MM-dd") + "' AND EntryDate <= '" + dtTo.ToString("yyyy-MM-dd") + "' " + query + " ORDER BY Id DESC ");
        DataTable dt = SQLQuery.ReturnDataTable("SELECT GradeID,GradeName from ItemGrade where CategoryID in (Select CategoryID from ItemSubGroup where GroupID=2 AND ProjectID=1) ORDER BY [GradeName]");

        foreach (DataRow dr in dt.Rows)
        {
            string gradeId = Convert.ToString(dr["GradeID"]);
            dr1 = dt1.NewRow();
            dr1["ItemGrade"] = Convert.ToString(dr["GradeName"]);
            ...
            string query = " ProductID IN (Select ProductID from Products where CategoryID IN (SELECT CategoryID from Categories where GradeID='" + gradeId + "')) ";
            query+=" AND InvNo IN (Select InvNo from sales where CustomerID = '" + ddCustomer.SelectedValue + "' and DeliveryDate >= '" + dtFrom.ToString("yyyy-MM-dd") + "' AND DeliveryDate <= '" + dtTo.ToString("yyyy-MM-dd") + "')  ";
    ...
    protected void GVrpt_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        {
            e.Row.Cells[1].Text = "Total Plastic Items: ";
    ...
    {
        exportExcel(LoadGridData(), "Sales-by-Items-Grade-Report");
    }
