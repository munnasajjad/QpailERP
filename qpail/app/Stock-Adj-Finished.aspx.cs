...
public partial class app_Stock_Adj_Finished : System.Web.UI.Page
{
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                txtWeight.Text = Convert.ToString(Convert.ToDecimal(txtUnitWeight.Text) * Convert.ToDecimal(txtQty.Text) / 1000M);
            }
            else
            {
                txtUnitWeight.Text = Convert.ToString(Convert.ToDecimal(txtWeight.Text) / (Convert.ToDecimal(txtQty.Text) * 1000M));
            }
    ...
    private void InsertData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO StockinDetails (OrderID, SizeId, ProductID, BrandID, ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor, GodownID, LocationID, QtyBalance, DeliveredQty) VALUES (@OrderID, @SizeId, @ProductID, @BrandID, @ProductName, @UnitCost, @Quantity, @UnitWeight, '" + ltrUnit.Text + "', @ItemTotal, @TotalWeight, @EntryBy, @CompanyFor, '" + ddGodown.SelectedValue + "', '" + ddLocation.SelectedValue + "', '0','" + txtCurrentQty.Text + "'  )", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        cmd2.Parameters.AddWithValue("@UnitWeight", txtUnitWeight.Text);

        cmd2.Parameters.AddWithValue("@ItemTotal", "0");
        cmd2.Parameters.AddWithValue("@TotalWeight", Convert.ToDecimal(txtWeight.Text));
    ...
    {
        string lName = Page.User.Identity.Name.ToString();
        SqlCommand cmd = new SqlCommand("SELECT Id, ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, QtyBalance, DeliveredQty, TotalWeight FROM StockinDetails WHERE GodownID='" + ddGodown.SelectedValue + "' AND LocationID = '" + ddLocation.SelectedValue + "' AND EntryBy=@EntryBy AND OrderID='' ORDER BY Id", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    {
        ltrQty.Text = RunQuery.SQLQuery.ReturnString("SELECT ISNULL(SUM(QtyBalance),0) FROM StockinDetails WHERE OrderID='' AND GodownID='" + ddGodown.SelectedValue + "' AND LocationID = '" + ddLocation.SelectedValue + "'");
        txtUnitWeight.Text = RunQuery.SQLQuery.ReturnString("SELECT ISNULL(UnitWeight,0) FROM FinishedProducts WHERE SizeID ='" + ddSize.SelectedValue + "' AND ProductID ='" + ddProduct.SelectedValue + "' AND BrandID ='" + ddBrand.SelectedValue + "'");
    ...
    private void ExecuteInsert()
    {
        ...
        SqlCommand cmd = new SqlCommand("UPDATE StockinDetails SET OrderID='" + orderNo + "' WHERE  EntryBy=@EntryBy AND OrderID=''  AND GodownID='" + ddGodown.SelectedValue + "' AND LocationID = '" + ddLocation.SelectedValue + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        foreach (GridViewRow row in ItemGrid.Rows)
        {
            ...
            string customer = RunQuery.SQLQuery.ReturnString("SELECT CustomerID FROM CustomerBrands where (BrandID='" + brandID + "')");

            Label lbliQty = row.FindControl("lbliQty") as Label;
            Label lblUnitWeight = row.FindControl("lbliWeight") as Label;
            Label lbliQtyCurrent = row.FindControl("lbliQtyCurrent") as Label;
            ...
            if (lbliQtyCurrent.Text == "0")
            {
                detail = "Opening Stock";
            }
            decimal price = 0;// Inventory.LastNonprintedPrice(Purpose, itemType, Item);
            Stock.Inventory.SaveToStock("", orderNo, detail, lblEntryId.Text, sizeId, customer, brandID, "", "", productID, ProductName, "", ddGodown.SelectedValue, ddLocation.SelectedValue, "2", inQty, outQty, price, inWeight, outWeight, "", "Stock-in", "Adjustment", ddLocation.SelectedItem.Text, lName, Convert.ToDateTime(txtDate.Text).ToString("yyyy-MM-dd"));

            //Accounting.VoucherEntry.StockEntry(orderNo, detail, lblEntryId.Text, sizeId, brandID, productID, ProductName, ddGodown.SelectedValue, ddLocation.SelectedValue, "2", inQty, outQty, "0", "0", "", "Stock-in", "Adjustment", ddLocation.SelectedItem.Text, lName);
    ...
    {
        string lName = Page.User.Identity.Name.ToString();
        SqlCommand cmd = new SqlCommand("Delete StockinDetails WHERE EntryBy=@EntryBy AND OrderID=''  AND WarehouseID='" + ddGodown.SelectedValue + "' ", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Parameters.AddWithValue("@EntryBy", lName);
        cmd.Connection.Open();
        cmd.ExecuteNonQuery();
        cmd.Connection.Close();
    ...
    private void QtyinStock()
    {
        ...
        ltrCQty.Text = txtQty.Text;
        txtCurrentKg.Text = RunQuery.SQLQuery.ReturnString("Select  ISNULL(sum(InWeight)-Sum(OutWeight),0) FROM Stock WHERE  SizeId='" + ddSize.SelectedValue + "' AND BrandID='" + ddBrand.SelectedValue + "' AND ProductID='" + ddProduct.SelectedValue + "'  AND WarehouseID='" + ddGodown.SelectedValue + "' ");
        txtUnitWeight.Text = RunQuery.SQLQuery.ReturnString("Select ISNULL(UnitWeight,0) from FinishedProducts  WHERE SizeID ='" + ddSize.SelectedValue + "' AND ProductID ='" + ddProduct.SelectedValue + "' AND BrandID ='" + ddBrand.SelectedValue + "'");
