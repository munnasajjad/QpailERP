...
public partial class app_StockLoanIssueReceive : System.Web.UI.Page
{
    ...
    public string InvIDNo()
    {
        ...
        SqlCommand cmd = new SqlCommand("Select CONVERT(varchar, (ISNULL (max(Id),0)+ 1 )) from StockLoanTakenGivenMaster where EntryDate>=@EntryDate", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    protected void btnAdd_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            else
            {
                ...
                SQLQuery.ExecNonQry("Delete StockLoanTakenGivenDetails where (id ='" + lblOrderID.Text + "')");
                InsertData();
                btnAdd.Text = "Add";
    ...
    private void InsertData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO StockLoanTakenGivenDetails (StockType, purpose, ItemType, OrderID, ProductID, ProductName, Manufacturer, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor, GodownID, LocationID, QtyBalance, DeliveredQty, Spec) " +
        ...
        {
            cmd2 = new SqlCommand(@"INSERT INTO StockLoanTakenGivenDetails (StockType, purpose, ItemType, OrderID, SizeId, ProductID, Customer, BrandID, ProductName, Color, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor, GodownID, LocationID, QtyBalance, DeliveredQty)" +
    ...
    private void ExecuteUpdate()
    {
        ...
        int balance = Convert.ToInt32(txtQty.Text) - Convert.ToInt32(ltrCQty.Text);
        SqlCommand cmd2 = new SqlCommand("UPDATE StockLoanTakenGivenDetails SET SizeId='" + ddGroup.SelectedValue + "', ProductID='" + ddProduct.SelectedValue + "', BrandID='" + ddSubGroup.SelectedValue + "'," +
    ...
    {
        string lName = Page.User.Identity.Name.ToString();
        //SqlCommand cmd = new SqlCommand(@"SELECT  Id, (SELECT  [Purpose] FROM [Purpose] WHERE [pid]= StockLoanTakenGivenDetails.Purpose) AS Purpose, 
        //                                    ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, QtyBalance, DeliveredQty, UnitWeight, 
        //                                    (SELECT  [Company] FROM [Party] WHERE [PartyID]= StockLoanTakenGivenDetails.Customer) AS Customer,
        //                                    (SELECT [BrandName] FROM [CustomerBrands] WHERE BrandID=StockLoanTakenGivenDetails.BrandID) AS BrandID,
        //                                    (SELECT [BrandName] FROM [Brands] WHERE BrandID=StockLoanTakenGivenDetails.SizeId) AS SizeId,
        //                                    (SELECT [DepartmentName] FROM [Colors] WHERE Departmentid=StockLoanTakenGivenDetails.Color) AS Color,
        //                                    (SELECT [Spec] FROM [Specifications] WHERE id=StockLoanTakenGivenDetails.Spec) AS Spec
        //                                    FROM StockLoanTakenGivenDetails WHERE GodownID='" + ddGodown.SelectedValue + "' AND EntryBy=@EntryBy AND OrderID='' AND StockType='Raw'  ORDER BY Id", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        SqlCommand cmd = new SqlCommand(@"SELECT  Id, ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, QtyBalance, DeliveredQty, UnitWeight, Manufacturer
                                            FROM StockLoanTakenGivenDetails WHERE GodownID='" + ddGodown.SelectedValue + "' AND LocationID = '" + ddLocation.SelectedValue + "' AND EntryBy=@EntryBy AND OrderID='' AND StockType='Raw'  ORDER BY Id", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    {
        ltrQty.Text = SQLQuery.ReturnString("Select ISNULL(SUM(QtyBalance),0) from StockLoanTakenGivenDetails where OrderID='' AND GodownID='" + ddGodown.SelectedValue + "' AND LocationID = '" + ddLocation.SelectedValue + "'") + "pcs, " +
            SQLQuery.ReturnString("Select ISNULL(SUM(UnitWeight),0) from StockLoanTakenGivenDetails where OrderID='' AND GodownID='" + ddGodown.SelectedValue + "' AND LocationID = '" + ddLocation.SelectedValue + "'") + "kg";
        //QtyinStock();
    }
    protected void ItemGrid_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            string OrderID = SQLQuery.ReturnString("Select OrderID from StockLoanTakenGivenDetails where Id='" + lblItemCode.Text + "'");
            //string deliveredQty = SQLQuery.ReturnString("Select SUM(DeliveredQty) from StockLoanTakenGivenDetails where OrderID='" + OrderID + "'");

            //if (deliveredQty == "0")
            //{
            SqlCommand cmd7 = new SqlCommand("DELETE StockLoanTakenGivenDetails WHERE Id=" + lblItemCode.Text + "", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    private void ExecuteInsert()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO StockLoanTakenGivenMaster (OrderID, OrderDate, GodownID, GodownName, LocationID, LocationName, ToGodownID, ToLocationID, Remarks, TotalQty, EntryBy, ProjectId)" +
        ...
        SqlCommand cmd = new SqlCommand("UPDATE StockLoanTakenGivenDetails SET OrderID='" + orderNo + "' WHERE  EntryBy=@EntryBy AND OrderID=''  AND StockType='Raw'  AND GodownID='" + ddGodown.SelectedValue + "' AND LocationID = '" + ddLocation.SelectedValue + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        foreach (GridViewRow row in ItemGrid.Rows)
        {
            ...
            string purpose = SQLQuery.ReturnString("SELECT Purpose FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')");
            string sizeId = SQLQuery.ReturnString("SELECT SizeID FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')");
            string brandID = SQLQuery.ReturnString("SELECT brandID FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')");
            string productID = SQLQuery.ReturnString("SELECT productID FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')");
            string ProductName = SQLQuery.ReturnString("SELECT ProductName FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')");
            string detail = SQLQuery.ReturnString("SELECT Manufacturer FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')");

            string itemType = SQLQuery.ReturnString("SELECT ItemType FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')");
            string customer = SQLQuery.ReturnString("SELECT Customer FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')");
            string color = SQLQuery.ReturnString("SELECT Color FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')");
            string spec = SQLQuery.ReturnString("SELECT spec FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')");
            decimal price = Convert.ToDecimal(SQLQuery.ReturnString("SELECT UnitCost FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "')"));
            ...
            //string companyFor = SQLQuery.ReturnString("SELECT Company from Party where PartyID=(Select CompanyFor FROM StockLoanTakenGivenDetails where (Id='" + lblEntryId.Text + "'))");
    ...
    {
        string lName = Page.User.Identity.Name.ToString();
        SqlCommand cmd = new SqlCommand("Delete StockLoanTakenGivenDetails WHERE EntryBy=@EntryBy AND OrderID=''  AND WarehouseID='" + ddGodown.SelectedValue + "' ", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Parameters.AddWithValue("@EntryBy", lName);
        cmd.Connection.Open();
        cmd.ExecuteNonQuery();
        cmd.Connection.Close();
    ...
    {
        SqlCommand cmd = new SqlCommand(@"SELECT ItemType, ProductID, SizeId, Customer, BrandID, Color, Quantity, UnitWeight, GodownID, LocationID, Manufacturer, UnitCost FROM [StockLoanTakenGivenDetails] WHERE Id='" + entryID + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Connection.Open();
    ...
    protected void ddRemark_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        BindGodownDropdown();
    }

    private void BindGodownDropdown()
    {
        if (ddRemark.SelectedValue == "LT")
        {
            SQLQuery.PopulateDropDown("SELECT [WID], [StoreName] FROM [Warehouses] WHERE IsSisterConcern='1'", ddFromGodown, "WID", "StoreName");
            SQLQuery.PopulateDropDown("SELECT [WID], [StoreName] FROM [Warehouses] WHERE IsSisterConcern<>'1'", ddGodown, "WID", "StoreName");
        }
        else
        {
            SQLQuery.PopulateDropDown("SELECT [WID], [StoreName] FROM [Warehouses] WHERE IsSisterConcern='1'", ddGodown, "WID", "StoreName");
