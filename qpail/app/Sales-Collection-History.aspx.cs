...
public partial class app_Sales_Collection_History : System.Web.UI.Page
{
    ...
    private DataTable search()
    {
        try
        {
            ...
            {
                customer = " AND InvNo IN (Select InvNo from Sales where CustomerID='" + ddCustomer.SelectedValue + "') ";
            ...
            if (txtInvDate.Text != "")
            {
                try
                {
                    ...
                    invDate = " AND  (PaymentDate >= '" + invDate + "')";
                }
                catch (Exception) { }
            }
            ...
            if (txtPODate.Text != "")
            {
                try
                {
                    ...
                    poDate = " AND  (PaymentDate <= '" + poDate + "')";
                }
                catch (Exception) { }
            }
            
            string query = customer + invNo + invDate + poNo + poDate;
            string url = SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") + "XerpReports/frmInvoice.aspx?inv=";
            DataSet ds = SQLQuery.ReturnDataSet(@"SELECT CID, CollectionNo, (Select ChequeNo from Cheque where TrID=CollectionInvoices.CollectionNo) AS ChqNo, 
                                                (Select ChqDate from Cheque where TrID=CollectionInvoices.CollectionNo) AS ChqDate, 
                                                (Select BankName from Banks where BankId =(Select ChqBank from Cheque where TrID=CollectionInvoices.CollectionNo)) AS BankName, 
                                                InvNo, '" + url + "'+InvNo as link,  InvoiceDate, MaturityDate, PaymentDate, OverdueDays," +
                                                " InvoiceTotal, VATAmount, PayableAmount, TDSRate, TdsVatIncluded, TDSAmount, VDSrate, VdsVatIncluded, VDSAmount, NetPayable," +
                                                " PreCollectedAmount, CollectedAmount FROM [CollectionInvoices] WHERE IsApproved<>0 " + query + " ORDER BY PaymentDate desc, CID desc");
