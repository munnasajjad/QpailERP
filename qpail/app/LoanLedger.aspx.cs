...
public partial class app_LoanLedger : System.Web.UI.Page
{
    ...
    {
        string customer = " "; string party = " where Id ='" + ddLoanTypes.SelectedValue + "'";

        if (ddLoanTypes.SelectedValue != "---ALL---")
        {
            customer = " AND HeadID ='" + ddLoanTypes.SelectedValue + "'";
        }

        decimal opBal = Convert.ToDecimal(SQLQuery.ReturnString("SELECT Rcvamount FROM BankLoan " + party));
        ...
        if (recordcount > 0)
        {
            do
            {
                ...
                string link = SQLQuery.ReturnString("Select TOP(1) SerialNo from VoucherTmp where VoucherNo='" + inv + "' AND (Head5Dr='" + ddLoanTypes.SelectedValue + "' OR Head5Cr='" + ddLoanTypes.SelectedValue + "') AND (Amount= '" + debt + "' OR Amount= '" + credit + "') ");
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                //LoadGridData();
                string bankId = ddCode.SelectedValue;
                ...
                string url = SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") +
                    "XerpReports/FormLoansLedger.aspx?Id=" + bankId + "&dateFrom=" + dateFrom + "&dateTo=" + dateTo;
    ...
    {
        SqlCommand cmd = new SqlCommand("SELECT CollectionNo, CollectionDate, PartyName, PaidAmount, ChqDetail, ChqDate FROM Collection WHERE (IsApproved = 'p') and PartyName='" + ddLoanTypes.SelectedValue + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Connection.Open();
    ...
    protected void btnPrint_Click(object sender, EventArgs e)
    {
        if (ddLoanTypes.Items.Count > 0)
        {
            ...
            htmlBody += "<i>Account Name: " + ddLoanTypes.SelectedItem.Text + "</i><br/>";
