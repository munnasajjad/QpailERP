...
public partial class app_LedgerControl : System.Web.UI.Page
{
    ...
    private void LoadGridData()
    {
        ...
        SqlCommand cmd2 = new SqlCommand("SELECT [EntryDate] as TrDate, [VoucherRowDescription] as Description, [VoucherDR] As Dr, [VoucherCR] As Cr, [VoucherNo] As Balance FROM [VoucherDetails] WHERE ([AccountsHeadID] IN (Select AccountsHeadID from HeadSetup where ControlAccountsID=@ControlAccountsID)) and  EntryDate >= @DateFrom AND EntryDate <= @DateTo  AND ISApproved<>'C' ORDER BY EntryDate, [SerialNo] ASC", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd2.Parameters.Add("@ControlAccountsID", SqlDbType.VarChar).Value = ddParties.SelectedValue;
        ...
        if (isLiability == "02")
        {
            decimal opBal = Convert.ToDecimal(RunQuery.SQLQuery.ReturnString("SELECT  isnull(sum(OpBalCr),0) - isnull(sum(OpBalDr),0)  FROM [HeadSetup] WHERE ([AccountsHeadID]  IN (Select AccountsHeadID from HeadSetup where ControlAccountsID= '" + ddParties.SelectedValue + "'))"));
            decimal preBal = Convert.ToDecimal(RunQuery.SQLQuery.ReturnString("SELECT  isnull(sum(VoucherCR),0) - isnull(sum(VoucherDR),0) FROM [VoucherDetails] WHERE ([AccountsHeadID]  IN (Select AccountsHeadID from HeadSetup where ControlAccountsID= '" + ddParties.SelectedValue + "')) and   EntryDate < '" + Convert.ToDateTime(txtDateFrom.Text).ToString("yyyy-MM-dd") + "'  and ISApproved='A'"));
            ...
            //get closing balance
            currBal = Convert.ToDecimal(RunQuery.SQLQuery.ReturnString("SELECT  isnull(sum(VoucherCR),0) - isnull(sum(VoucherDR),0) FROM [VoucherDetails] WHERE ([AccountsHeadID]  IN (Select AccountsHeadID from HeadSetup where ControlAccountsID= '" + ddParties.SelectedValue + "')) and   EntryDate < '" + Convert.ToDateTime(txtDateTo.Text).AddDays(+1).ToString("yyyy-MM-dd") + "'  and ISApproved='A'"));
        ...
        else
        {
            ...
            SqlCommand cmd2x = new SqlCommand("SELECT  isnull(sum(OpBalDr),0) - isnull(sum(OpBalCr),0)  FROM [HeadSetup] WHERE ([AccountsHeadID]  IN (Select AccountsHeadID from HeadSetup where ControlAccountsID= @HeadName))", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
            ...
            SqlCommand cmd2y = new SqlCommand("SELECT  isnull(sum(VoucherDR),0) - isnull(sum(VoucherCR),0) FROM [VoucherDetails] WHERE ([AccountsHeadID]  IN (Select AccountsHeadID from HeadSetup where ControlAccountsID= @HeadName)) and   EntryDate < @DateFrom  and ISApproved='A'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
            ...
            //get closing balance
            SqlCommand cmd2z = new SqlCommand("SELECT isnull(sum(VoucherDR),0)-isnull(sum(VoucherCR),0) as balance FROM [VoucherDetails] WHERE ([AccountsHeadID]  IN (Select AccountsHeadID from HeadSetup where ControlAccountsID= @HeadName)) and EntryDate < @DateTo and ISApproved='A'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
    ...
    {
        string htmlBody = "<div  style='text-align:center'><h2>TAHER FOOD INDUSTRIES</h2><h3>1285 D.T. Road, Pahartali, Chittagong-4202, Bangladesh.</h3></div><hr/>";
        htmlBody += "<div  style='text-align:center'><b><u><h4>Customer Credit List</h4></u></b></div>";
        //htmlBody += "<div  style='text-align:center'><b><h4>Sales Zone: " + ddCustomer.SelectedItem.Text + "</h4></b></div><br/>";
        //htmlBody += "<i>Sales Zone: " + ddCustomer.SelectedItem.Text + "</i><br/>";

        //htmlBody += "<i>Date: from " + txtInvDate.Text + " to " + txtPODate.Text + "</i><br/><br/>";
        ...
        string savePath = Server.MapPath("../App/Docs/Print/") + "Customer-Credit-List.pdf";
        File.WriteAllBytes(savePath, pdfBytes);

        string url = "../App/Docs/Print/Customer-Credit-List.pdf";
        Page.ClientScript.RegisterStartupScript(base.GetType(), "Open Window", "window.open('" + url + "' , 'no', 'height=680, width= 1024, top=300, left=500, location=1, scrollbars=1, resizable=1');", true);
