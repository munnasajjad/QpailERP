...
public partial class app_InkStockRegister : System.Web.UI.Page
{
    ...
    private void LoadGridData()
    {
        try
        {
            ...
            string query = @"SELECT Q1.ItemCode, Q1.SizeID, CONCAT(P.ItemName,' (',Q1.SizeID,')') AS ItemName, 0 AS OpeningStock, ISNULL(Q1.Received,0) AS Received, ISNULL(Q2.Issued,0) AS Issued, (ISNULL(Q1.Received,0)-ISNULL(Q2.Issued,0)) AS Balanced 
FROM (SELECT ItemCode, SizeID, COUNT(InKg) AS Received FROM tblFifo WHERE InDate>= '" + dateFrom + "' AND InDate<= '" + dateTo + "' GROUP BY ItemCode, SizeID) AS Q1 " +
                           "LEFT JOIN (SELECT ItemCode, SizeID, COUNT(InKg) AS Issued FROM dbo.tblFifo AS tblFifo_1 WHERE (OutTypeId<> '') AND(OutType <> '') AND OutDate>= '" + dateFrom + "' AND OutDate<= '" + dateTo + "' GROUP BY ItemCode, SizeID) AS Q2 ON Q1.ItemCode = Q2.ItemCode INNER JOIN Products AS P ON Q1.ItemCode = P.ProductID WHERE Q1.ItemCode = '" + ddInkName.SelectedValue + "' AND Q1.SizeID = '" + ddSpecification.SelectedValue + "'" +
                           "UNION (SELECT F.ItemCode, F.SizeID, CONCAT(P.ItemName, ' (', F.SizeID, ')') AS ItemName, COUNT(F.InKg) AS OpeningStock, 0 AS Received, 0 AS Issued, 0 AS Balanced FROM tblFifo AS F INNER JOIN Products AS P ON P.ProductID = F.ItemCode WHERE F.InDate < '" + dateFrom + "' AND F.OutType = '' AND F.OutTypeId = '' AND F.ItemCode = '" + ddInkName.SelectedValue + "' AND F.SizeID = '" + ddSpecification.SelectedValue + "' GROUP BY F.ItemCode, F.SizeID, P.ItemName)";
            SqlCommand cmd2 =
    ...
    private void LoadSpecList()
    {
        ...
        string gQuery = "SELECT [id], [spec] FROM [Specifications] WHERE  CAST(id AS nvarchar) IN (SELECT DISTINCT spec FROM stock WHERE ProductID='" + ddInkName.SelectedValue + "') ORDER BY [spec]";
        SQLQuery.PopulateDropDown(gQuery, ddSpecification, "id", "spec");
