...
public partial class app_Sale_Grade_Company : System.Web.UI.Page
{
    ...
    private DataTable LoadGridData()
    {
        ...
        foreach (DataRow dr in dt.Rows)
        {
            ...
            string query = " ProductID IN (Select ProductID from Products where CategoryID IN (SELECT CategoryID from Categories where GradeID='" + ddGrade.SelectedValue + "')) ";
            if (chkAll.Checked)
            {
                query = " ProductID IN (Select ProductID from Products where CategoryID IN (SELECT CategoryID from Categories where GradeID<>'20')) ";
            }
    ...
    protected void GVrpt_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        else if (e.Row.RowType == DataControlRowType.Footer)
        {
            ...
            e.Row.Cells[4].Text = Convert.ToString(vat);
            e.Row.Cells[5].Text = Convert.ToString(TotalSales); //String.Format("{0:c}", TotalSales);
            e.Row.Cells[6].Text = Convert.ToString(TotalWeight);  //String.Format("{0:c}", TotalSales);
        }
    }
    ...
    {
        exportExcel(LoadGridData(), "Sales-Grade-by-Company-Report");
    }
