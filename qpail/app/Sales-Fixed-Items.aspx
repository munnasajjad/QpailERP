<%@ Page Title="Sales" Language="C#" MasterPageFile="~/app/MasterPage.master" AutoEventWireup="true" CodeFile="Sales-Fixed-Items.aspx.cs" Inherits="app_Sales_Fixed_Items" %>
  ...
  cursor: pointer; /* Add a pointer on hover */
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#<%=txtSdAmount.ClientID%>').attr('readonly', true);
        ...
        function calInvTotal() {
            ...
            var sdRate = $('#<%=txtSdRate.ClientID%>').val();
            //sdRate = Empty2Zero(sdRate);
            //console.log(sdRate);
            //if (isNaN(sdRate)) sdRate = 0;
            var disc = $('#<%=txtDiscount.ClientID%>').val();
            var vat = $('#<%=txtVat.ClientID%>').val();

            var sdAmount = (parseFloat(ttl) * parseFloat(sdRate) / 100);
            $('#<%=txtSdAmount.ClientID%>').val(sdAmount.toString());
            //var totalWithSd = parseFloat(ttl) - parseFloat(disc) + (parseFloat(ttl) * parseFloat(sdRate) / 100);

            //if (isNaN(sdAmount)) sdAmount = 0;
            
            var pay = parseFloat(ttl) - parseFloat(disc) + ((parseFloat(ttl) + parseFloat(sdAmount)*1) * parseFloat(vat) / 100);
            $('#<%=txtPay.ClientID%>').val(pay.toString());
            $('.vatamt').html("Total VAT Amount: " + (parseFloat(ttl) + parseFloat(sdAmount) * 1) * parseFloat(vat) / 100);
        }
                    ...
                    <div class="portlet box blue">
                        ...
                        <div class="portlet-body form">
                            ...
                            <div class="form-horizontal" role="form">
                                ...
                                <div class="control-group">
                                    <label class="control-label full-wdth">SD Rate | SD Amount:</label>
                                    <div class="controls">
                                        <asp:TextBox ID="txtSdRate" runat="server" CssClass="form-control" placeholder="Suplimentary Duty Rate" onkeyup="calInvTotal()" style="width: 30% !important" Text="0" />
                                        <asp:FilteredTextBoxExtender ID="ftbSdRate" runat="server" FilterType="Custom, Numbers" ValidChars="0123456789" TargetControlID="txtSdRate" />
                                        <asp:TextBox ID="txtSdAmount" runat="server" CssClass="form-control" placeholder="Suplimentary Duty (SD)" style="width: 70% !important" Text="0" />
                                    </div>
                                </div>
                                                ...
                                                <asp:Panel ID="pnlAdd" runat="server" DefaultButton="btnAdd">
                                                    ...
                                                    <div class="col-md-4 hidden">
                                                        <div class="control-group">
                                                            ...
                                                            <asp:DropDownList ID="ddProduct" runat="server" DataSourceID="SqlDataSource8" AutoPostBack="true" OnSelectedIndexChanged="ddProduct_SelectedIndexChanged"
