...
public partial class app_Finished_Items : System.Web.UI.Page
{
    ...
    {
        SQLQuery.PopulateDropDown("SELECT [BrandID], [BrandName] FROM [CustomerBrands] WHERE (([CustomerID] = '" + ddCustomer.SelectedValue + "') ) ORDER BY BrandName", ddBrand, "BrandID", "BrandName");
    }
    ...
    public string ItemExist()
    {
        ...
        try
        {
            DataTable dtx = SQLQuery.ReturnDataTable(@"SELECT pid, ProductCode, ProductName, AccHeadIdContainer, AccHeadIdLid, AccHeadIdHandle, AccHeadIdSticker, UnitPrice, UnitWeight FROM FinishedProducts WHERE SizeID ='" + ddSize.SelectedValue + "' AND ProductID ='" + ddProduct.SelectedValue + "' AND BrandID ='" + ddBrand.SelectedValue + "'");
