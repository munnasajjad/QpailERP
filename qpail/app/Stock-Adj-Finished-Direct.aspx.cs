...
public partial class app_Stock_Adj_Finished_Direct : System.Web.UI.Page
{
    ...
    private void InsertData()
    {
        ...
        if (Convert.ToDecimal(txtCurrentQty.Text) > 0)
        {
            txtWeight.Text =
                Convert.ToString(Convert.ToDecimal(txtCurrentKg.Text)/Convert.ToDecimal(txtCurrentQty.Text)*
        ...
        SqlCommand cmd2 = new SqlCommand("INSERT INTO StockinDetails (OrderID, SizeId, ProductID, BrandID, ProductName, UnitCost, Quantity, UnitWeight, UnitType, ItemTotal, TotalWeight, EntryBy, CompanyFor,  GodownID, QtyBalance, DeliveredQty) VALUES (@OrderID, @SizeId, @ProductID, @BrandID, @ProductName, @UnitCost, @Quantity, @UnitWeight, '" + ltrUnit.Text + "', @ItemTotal, @TotalWeight, @EntryBy, @CompanyFor, '" + ddGodown.SelectedValue + "', '0', '" + txtCurrentQty.Text + "'  )", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        cmd2.Parameters.AddWithValue("@UnitWeight", txtCurrentKg.Text);
    ...
    private void ExecuteInsert()
    {
        ...
        SqlCommand cmd = new SqlCommand("UPDATE StockinDetails SET OrderID='" + orderNo + "' WHERE  EntryBy=@EntryBy AND OrderID=''  AND GodownID='" + ddGodown.SelectedValue + "' ", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        ...
        foreach (GridViewRow row in ItemGrid.Rows)
        {
            ...
            Label lblOldWeight = row.FindControl("lblUWeight") as Label;
            Label lbliQtyCurrent = row.FindControl("lbliQtyCurrent") as Label;
            Label lblSetWeight = row.FindControl("lbliWeight") as Label;

            int cQty = Convert.ToInt32(lbliQtyCurrent.Text);
            int setQty = Convert.ToInt32(lbliQty.Text);
            int inQty = 0; int outQty = 0;



            decimal inWeight = 0; decimal outWeight = 0;
            decimal cWt = Convert.ToDecimal(lblOldWeight.Text);
            decimal setWt = Convert.ToDecimal(lblSetWeight.Text);
