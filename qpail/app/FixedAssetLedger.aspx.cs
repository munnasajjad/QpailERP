...
public partial class app_FixedAssetLedger : System.Web.UI.Page
{
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            txtDatefrom1.Text = DateTime.Now.ToString("01/MM/yyyy");
    ...
    {
         if (ddlcinv.SelectedValue != "")
            {
            
                if (txtDatefrom1.Text != txtDateto.Text)
                
                {
                    ...
                    string id = SQLQuery.ReturnString("SELECT ProductID FROM Products WHERE  ItemName ='" +
                                                      ddProduct.SelectedValue + "' ");
                    //string item = ddProduct.SelectedValue;
                    string purchaseDateSearch =
                        SQLQuery.ReturnString("SELECT InDate FROM FixedAssets   WHERE  LCNoFTT ='" +
                                              ddlcinv.SelectedValue + "' ");
                    string purchasedate = Convert.ToDateTime(purchaseDateSearch).ToString("yyyy-MM-dd");
                    string purchaseAmount = SQLQuery.ReturnString(
                        "SELECT SUM(PurchaseCost)  FROM FixedAssets  WHERE  LCNoFTT ='" + ddlcinv.SelectedValue + "' ");

                    string datefrom = Convert.ToDateTime(txtDatefrom1.Text).ToString("yyyy-MM-dd");
                    string dateto = Convert.ToDateTime(txtDateto.Text).ToString("yyyy-MM-dd");

                    // string depRate = SQLQuery.ReturnString("SELECT Depreciation FROM Products  WHERE  ProductID ='" + ddProduct.SelectedValue + "' ");
                    string depPercent = SQLQuery.ReturnString(
                        "SELECT Convert(int, Depreciation) FROM Products Where ProductID ='" + ddProduct.SelectedValue +
                        "'");
                    if (depPercent == "" || depPercent == "0")
                    {

                        depPercent = SQLQuery.ReturnString(
                            "SELECT Depreciationvalue FROM ItemGroup WHERE GroupSrNo= '" + ddGroup.SelectedValue + "'");
                    }

                    string urlx = SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") +
                                  "XerpReports/FormFixedAssetLedger.aspx?item=" + ddProduct.SelectedValue + "&lcinv=" +
                                  ddlcinv.SelectedValue + "&pdate=" + purchasedate + "&cost=" + purchaseAmount +
                                  "&depRate=" + depPercent + "&date1=" + datefrom + "&date2=" + dateto;
                    if1.Attributes.Add("src", urlx);
                }
                else
                {
                lblMsg2.Attributes.Add("class", "xerp_warning");
                lblMsg2.Text = "ERROR: Date-From and Date-To Shouldn't be same!";
            }
            }
        else
        {
            lblMsg2.Attributes.Add("class", "xerp_warning");
            lblMsg2.Text = "ERROR: LC No/Involve No not found!";
        }
    }
