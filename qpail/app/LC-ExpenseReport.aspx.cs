...
public partial class app_LC_ExpenseReport : System.Web.UI.Page
{
    ...
    private void LoadGridData()
    {
        ...
        decimal opBal = 0;
        if (ddParty.SelectedValue != "---ALL---" && ddParty.SelectedValue != "")
        {
            customer = " AND HeadID ='" + ddParty.SelectedValue + "'";
            party = " where PartyID ='" + ddParty.SelectedValue + "'";
        ...
        //                                  ddParty.SelectedValue + "' AND IsActive=1 AND InvDate<'" + lastMaturityDate + "' AND InvDate<'" + dateFrom + "'"));

        //    closeBal = opBal + Convert.ToDecimal(SQLQuery.ReturnString("Select ISNULL(SUM(DueAmount),0) from Sales where CustomerID='" +
        //                                  ddParty.SelectedValue + "' AND IsActive=1 AND InvDate<'" + lastMaturityDate + "' AND InvDate<='" + dateTo + "'"));
        //}     

        //if (ddStatus.SelectedValue == "Immatured")
        //{
        //    currBal = Convert.ToDecimal(SQLQuery.ReturnString("Select ISNULL(SUM(DueAmount),0) from Sales where CustomerID='" + ddParty.SelectedValue + "'  AND IsActive=1 AND InvDate>'" +
        //                                  lastMaturityDate + "' AND InvDate<'" + dateFrom + "'"));
        //    closeBal = Convert.ToDecimal(SQLQuery.ReturnString("Select ISNULL(SUM(DueAmount),0) from Sales where CustomerID='" + ddParty.SelectedValue + "'  AND IsActive=1 AND InvDate>'" +
        ...
        cmd2.Parameters.Add("@HeadName", SqlDbType.VarChar).Value = ddParty.SelectedValue;
    ...
    protected void btnShow_Click(object sender, EventArgs e)
    {
        try
        {
            if (Convert.ToDateTime(txtDateFrom.Text) <= Convert.ToDateTime(txtDateTo.Text))
            {
                ...
                string partyID = ddParty.SelectedValue;
                string reportType = ddType.SelectedValue;
                string dateFrom = Convert.ToDateTime(txtDateFrom.Text).ToString("yyyy-MM-dd");
                string dateTo = Convert.ToDateTime(txtDateTo.Text).ToString("yyyy-MM-dd");
                
                string url = SQLQuery.ReturnString("Select DefaultLink from Settings_Project where sid=1") +
                    "XerpReports/FormLCExpenseReport.aspx?PartyId=" + partyID + "&ReportType=" + reportType;
    ...
    {
        SqlCommand cmd = new SqlCommand("SELECT CollectionNo, CollectionDate, PartyName, PaidAmount, ChqDetail, ChqDate FROM Collection WHERE (IsApproved = 'p') and PartyName='" + ddParty.SelectedValue + "'", new SqlConnection(ConfigurationManager.ConnectionStrings["LocalSqlServer"].ConnectionString));
        cmd.Connection.Open();
    ...
    protected void ddParty_OnSelectedIndexChanged(object sender, EventArgs e)
    {
        LoadGridData();
    }
    ...
    {
        string url = "./rptLedger.aspx?party=" + ddParty.SelectedValue + "&dt1=" + txtDateFrom.Text + "&dt2=" + txtDateTo.Text;
